...
params:
  csv_path: "log_20170418_104447.csv"
  timeout_ms: 600000
output:
  html_document:
    code_folding: hide
...
if (n_rows_empty > 0) warning(paste(c(n_rows_empty, " rows contain missing values (no timeout, no exception)")))
d_exceptions <- d %>% filter(str_detect(d$Safe, "\\[EX\\]"))
n_rows_exception <- nrow(d_exceptions)
if (n_rows_exception > 0) {
  warning(paste(c(n_rows_exception, " rows contain exceptions that are converted to NAs.")))
  knitr::kable(d_exceptions %>%
                 group_by(Type = Safe) %>%
                 summarise(Configs = toString(unique(Config)), Count = n()))
}
...
- There are **`r n_models_total` models** and **`r n_configs_total` configurations**, giving **`r n_meas_total` measurements**, with **`r n_meas_succ` being successful** with the timeout limit of `r params$timeout_ms/1000` s (`r n_meas_succ / n_meas_total * 100`% success rate).
...
- With the repeated measurements included, there are a total number of **`r n_meas_rep_total` measurement points** with **`r n_meas_rep_succ` successful** executions.
