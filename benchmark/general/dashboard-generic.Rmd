...
gaugeSectorsPercent <- function(min, max, warn, succ) {
...
ex_rows <- nrow(d_exceptions)
to_rows <- sum(str_detect(d$Safe, "\\[TO\\]"), na.rm = T)
...
### Measurements
```{r}
valueBox(n_models_total * n_configs_total, icon = "fa-tasks")
```

### Success rate
```{r}
gauge(nrow(d_model_config %>% filter(Succ)), min = 0, max = n_models_total * n_configs_total,
      gaugeSectorsPercent(0, n_models_total * n_configs_total, 50, 75))
```

### Models verified (at least one config)
```{r}
gauge(n_models_succ, min = 0, max = n_models_total,
      gaugeSectorsPercent(0, n_models_total, 50, 75))
```
...
```{r}
gauge(n_meas_succ, min = 0, max = n_meas_total,
      gaugeSectorsPercent(0, n_meas_total, 50, 75))
```

### Missing values & exceptions
```{r}
gauge(empty_rows + ex_rows, min = 0, max = n_meas_total,
      gaugeSectorsPercent(0, n_meas_total, 10, 1))
```

### Total time
```{r}
total_time_str <- paste(total_time$day, "d ", total_time$hour, "h ", total_time$minute, "m", sep = "")
valueBox(total_time_str, icon = "fa-clock-o")
```
...
gauge(max(d$TimeMs, na.rm = T) / 1000, min = 0, max = params$timeout_ms / 1000,
      gaugeSectorsPercent(0, params$timeout_ms / 1000, 95, 90))
```
...
There are **`r ex_rows`** exceptions.
...
config_max <- max(config_stats$N)
config_min <- min(config_stats$N)

model_stats <- d_model_config %>% group_by(Model) %>% summarise(N = sum(Succ))
model_max <- max(model_stats$N)
model_min <- min(model_stats$N)
```
...
```{r}
gauge(config_min, min = 0, max = n_models_total,
      gaugeSectorsPercent(0, n_models_total, 50, 75))
```

### Highest success rate of a config
```{r}
gauge(config_max, min = 0, max = n_models_total,
      gaugeSectorsPercent(0, n_models_total, 50, 75))
```

### Lowest success rate of a model
```{r}
gauge(model_min, min = 0, max = n_configs_total,
      gaugeSectorsPercent(0, n_configs_total, 50, 75))
```

### Highest success rate of a model
```{r}
gauge(model_max, min = 0, max = n_configs_total,
      gaugeSectorsPercent(0, n_configs_total, 50, 75))
```
...
### Number of models verified by a configuration
```{r}
ggplot(d_model_config) +
