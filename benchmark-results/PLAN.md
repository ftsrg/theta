...
**Next blocker (pre-existing, not introduced)**: with unions parsing, 56 of 70 sampled union-bearing tasks now reach `MallocFunctionPass` and die there on LP64 with `ClassCastException: ... is not of type (Bv 64), but (Bv 32)` — the malloc call's return type comes out 32-bit while the `__malloc` counter is pointer-width. Invisible on ILP32 (where both are `Bv32`). It does not reproduce on a minimal malloc/array/union program, so it needs its own investigation; it is now the top frontend-error class behind the grammar (`ParseCancellationException` in `assignmentExpression`, which is what still blocks most of aws-c-common — Phase 4, "handle with care").
