...
public class ExprITEEliminatorTests {
	...
	public void testSimple() {
		// if A then B else C
		assertEquals(eliminateITE(Ite(cA, cB, cC), manager), And(Or(Not(cA), cB), Or(cA, cC)));

		// if A then (if B then C else D) else E
		assertEquals(eliminateITE(Ite(cA, Ite(cB, cC, cD), cE), manager),
	...
	public void testUnary() {
		// !(if A then B else C)
		assertEquals(eliminateITE(Not(Ite(cA, cB, cC)), manager), Not(And(Or(Not(cA), cB), Or(cA, cC))));
	}

	@Test
	public void testBinary() {
		// A -> (if B then C else D)
		assertEquals(eliminateITE(Imply(cA, Ite(cB, cC, cD)), manager), Imply(cA, And(Or(Not(cB), cC), Or(cB, cD))));
		// (if B then C else D) -> A
		assertEquals(eliminateITE(Imply(Ite(cB, cC, cD), cA), manager), Imply(And(Or(Not(cB), cC), Or(cB, cD)), cA));
		// A = (if B then C else D)
		assertEquals(eliminateITE(Eq(cA, Ite(cB, cC, cD)), manager), Eq(cA, And(Or(Not(cB), cC), Or(cB, cD))));
		// X = (if A then Y else Z)
		assertEquals(eliminateITE(Eq(cX, Ite(cA, cY, cZ)), manager), And(Or(Not(cA), Eq(cX, cY)), Or(cA, Eq(cX, cZ))));
		// (if A then Y else Z) = X
		assertEquals(eliminateITE(Eq(Ite(cA, cY, cZ), cX), manager), And(Or(Not(cA), Eq(cY, cX)), Or(cA, Eq(cZ, cX))));
		// X = (if A then (if B then Y else Z) else T)
		assertEquals(eliminateITE(Eq(cX, Ite(cA, Ite(cB, cY, cZ), cT)), manager),
				And(Or(Not(cA), And(Or(Not(cB), Eq(cX, cY)), Or(cB, Eq(cX, cZ)))), Or(cA, Eq(cX, cT))));
		// (if A then (if B then Y else Z) else T) = X
		assertEquals(eliminateITE(Eq(Ite(cA, Ite(cB, cY, cZ), cT), cX), manager),
				And(Or(Not(cA), And(Or(Not(cB), Eq(cY, cX)), Or(cB, Eq(cZ, cX)))), Or(cA, Eq(cT, cX))));
		// (if A then X else Y) = (if B then Z else T)
		assertEquals(eliminateITE(Eq(Ite(cA, cX, cY), Ite(cB, cZ, cT)), manager),
	...
	public void testMultiary() {
		...
		assertEquals(eliminateITE(Or(cA, cB, Ite(cC, cD, cE)), manager), Or(cA, cB, And(Or(Not(cC), cD), Or(cC, cE))));
		// 1 = 2 + (if A then 3 else 4) + 5
		assertEquals(eliminateITE(Eq(i1, Add(i2, Ite(cA, i3, i4), i5)), manager),
	...
	public void testNothingHappening() {
		...
		for (final Expr<? extends BoolType> expr : expressions)
			assertEquals(expr, eliminateITE(expr, manager));
	}
}
