...
Theta is written in Java 8 using [Gradle](https://gradle.org/) as a build system and [Git](https://git-scm.com/) for version control. We are developing both on Windows and Linux.
...
* The projects are currently developed and tested with [Eclipse Neon](http://www.eclipse.org/neon/).
* Install a Gradle Eclipse plugin from the _Eclipse Marketplace_, e.g. the _Buildship: Eclipse Plug-ins for Gradle_ (search for _Buildship Gradle Integration_).
  * _Optional: Also install the Eclipse Groovy tooling from <https://github.com/groovy/groovy-eclipse/wiki>._
* To import the projects, choose _Import..._ / _Gradle Project_, specify the root directory as the repository directory and import them with the default _Gradle distribution_ (_Gradle wrapper (recommended)_).
