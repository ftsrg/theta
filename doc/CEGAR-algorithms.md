...
> **Note:** the per-tool availability remarks below ("available for CFA/XSTS") partly predate the XCFA-based tool (`xcfa-cli`), which by now supports most of the CFA options and several of the XSTS options (including the `EXPL_PRED_*` product domains). When in doubt, trust the tool's `--help` over this document.
...
* `EXPL_PRED_CART`, `EXPL_PRED_SPLIT`, `EXPL_PRED_BOOL` and `EXPL_PRED_COMBINED`: Product abstraction domains, available for XSTS and XCFA models. The set of control variables (marked with `ctrl`) are tracked explicitly while others are tracked by predicates (using the corresponding predicate domain). Variables can automatically be switched from predicate tracking to explicit tracking depending on the `--autoexpl` option (see below).
