...
* **Modules** — documentation living inside the subprojects. A module can have up to three pages:
    ...
    | `AGENTS.md` | `.../AGENTS.md` | how to *modify* the module: invariants, conventions, change recipes |

    Larger modules may also carry `AGENTS.md` files deeper inside (e.g. `common/core/type/`); those
...
!!! note "About the `USING.md` and `AGENTS.md` pages"
    ...
    `AGENTS.md` automatically when editing a module. They are deliberately terse and uneven in
    coverage. `README.md` pages are ordinary, hand-written documentation.

## Contributing

Documentation lives next to the code it describes: add a `README.md`, `USING.md`, or `AGENTS.md` to a subproject and it appears here automatically. See [Contributing](contributing.md) for the details and for how to preview the wiki locally.
