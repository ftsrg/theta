...
public class SequenceInterpolater extends AbstractCEGARStep implements Interpolater {
	...
	public Interpolant interpolate(final InterpolatedAbstractSystem system,
		...
		interpolatingSolver.add(markers[0], unroller.init(0)); // Initial
																// conditions
																// for the first
																// marker
		for (int i = 0; i < abstractCounterEx.size(); ++i) { // Loop through
			...
			for (final Expr<? extends BoolType> label : abstractCounterEx.get(i).getLabels())
				interpolatingSolver.add(markers[i], unroller.unroll(label, i)); // Assert
																				// labels
			if (i > 0)
				interpolatingSolver.add(markers[i], unroller.trans(i - 1)); // Assert
																			// transition
																			// relation
			interpolatingSolver.add(markers[i], unroller.inv(i)); // Assert
																	// invariants
		}

		// Set the last marker
		final NotExpr negSpec = system.getManager().getExprFactory().Not(system.getSTS().getProp());
		interpolatingSolver.add(markers[abstractCounterEx.size()],
				unroller.unroll(negSpec, abstractCounterEx.size() - 1)); // Property
																			// violation
