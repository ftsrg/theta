...
public class ClusteredRefiner extends AbstractCEGARStep implements Refiner<ClusteredAbstractSystem, ClusteredAbstractState> {
	...
	public ClusteredAbstractSystem refine(final ClusteredAbstractSystem system, final List<ClusteredAbstractState> abstractCounterEx,
		...
		final List<AndExpr> deadEndStates = getDeadEndStates(abstractCounterEx, traceLength, solver, system.getManager(), sts);

		// Bad states are not needed currently, they can be collected for debugging
		/// List<Model> badStates = getBadStates(abstractCounterEx, traceLength, solver, system.getManager(), sts);
	...
	private List<AndExpr> getDeadEndStates(final List<ClusteredAbstractState> abstractCounterEx, final int traceLength, final Solver solver,
			final STSManager manager, final STS sts) {
