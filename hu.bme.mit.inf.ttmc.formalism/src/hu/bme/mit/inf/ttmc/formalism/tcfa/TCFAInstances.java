...
public final class TCFAInstances {
	...
	public static TCFA fischer(final int id, final int ia, final int ib, final VarDecl<IntType> vlock) {
		...
		final VarDecl<RatType> vx = Var("x_" + id, Rat());

		final Expr<RatType> x = vx.getRef();
		...
		edge1.getStmts().add(Assign(vx, rzero));

		final TCFAEdge edge2 = tcfa.createEdge(l1, l2);
		edge2.getStmts().add(Assign(vx, rzero));
		...
		edge5.getStmts().add(Assign(vx, rzero));
