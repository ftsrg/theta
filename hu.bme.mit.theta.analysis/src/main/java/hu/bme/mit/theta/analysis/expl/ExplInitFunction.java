...
public final class ExplInitFunction
		implements InitFunction<ExplState, ExplPrec> {

	private final Solver solver;
	private final Expr<BoolType> initExpr;

	private ExplInitFunction(final Solver solver,
			final Expr<BoolType> initExpr) {
		this.solver = checkNotNull(solver);
		this.initExpr = checkNotNull(initExpr);
	}

	public static ExplInitFunction create(final Solver solver,
			final Expr<BoolType> initExpr) {
