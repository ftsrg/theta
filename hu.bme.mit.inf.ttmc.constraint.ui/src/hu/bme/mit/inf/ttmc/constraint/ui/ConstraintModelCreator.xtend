...
public class ConstraintModelCreator {
	...
	public def ConstraintModel create() {
		...
		for (basicConstraintDefinition : specification.basicConstraintDefinitions) {
			...
			val expr = ExprUtils.cast(inferrer, expression.toExpr, BoolType)
	...
	protected def dispatch Expr<? extends Type> toExpr(AddExpression expression) {
		val ops = expression.operands.map[ExprUtils.cast(inferrer, toExpr, ClosedUnderAdd)]
		Add(ops)
	}

	protected def dispatch Expr<? extends Type> toExpr(MultiplyExpression expression) {
		val ops = expression.operands.map[ExprUtils.cast(inferrer, toExpr, ClosedUnderMul)]
		Mul(ops)
	}

	protected def dispatch Expr<? extends Type> toExpr(UnaryMinusExpression expression) {
		val op = ExprUtils.cast(inferrer, expression.operand.toExpr, ClosedUnderNeg)
		Neg(op)
	}

	protected def dispatch Expr<? extends Type> toExpr(SubtractExpression expression) {
		val leftOp = ExprUtils.cast(inferrer, expression.leftOperand.toExpr, ClosedUnderSub)
		val rightOp = ExprUtils.cast(inferrer, expression.rightOperand.toExpr, ClosedUnderSub)
		Sub(leftOp, rightOp)
	}

	protected def dispatch Expr<? extends Type> toExpr(DivideExpression expression) {
		val leftOp = ExprUtils.cast(inferrer, expression.leftOperand.toExpr, RatType)
		val rightOp = ExprUtils.cast(inferrer, expression.rightOperand.toExpr, RatType)
		RatDiv(leftOp, rightOp)
	}

	protected def dispatch Expr<? extends Type> toExpr(DivExpression expression) {
		val leftOp = ExprUtils.cast(inferrer, expression.leftOperand.toExpr, IntType)
		val rightOp = ExprUtils.cast(inferrer, expression.rightOperand.toExpr, IntType)
	...
	protected def dispatch Expr<? extends Type> toExpr(NotExpression expression) {
		val op = ExprUtils.cast(inferrer, expression.operand.toExpr, BoolType)
		Not(op)
	}

	protected def dispatch Expr<? extends Type> toExpr(ImplyExpression expression) {
		val leftOp = ExprUtils.cast(inferrer, expression.leftOperand.toExpr, BoolType)
		val rightOp = ExprUtils.cast(inferrer, expression.rightOperand.toExpr, BoolType)
	...
	protected def dispatch Expr<? extends Type> toExpr(AndExpression expression) {
		val ops = expression.operands.map[ExprUtils.cast(inferrer, toExpr, BoolType)]
		And(ops)
	}
	...
	protected def dispatch Expr<? extends Type> toExpr(ArrayAccessExpression expression) {
		val array = ExprUtils.cast(inferrer, expression.operand.toExpr, ArrayType) as Expr<ArrayType<Type, Type>>
	...
	protected def dispatch Expr<? extends Type> toExpr(IfThenElseExpression expression) {
		val cond = ExprUtils.cast(inferrer, expression.condition.toExpr, BoolType)
	...
	private static class ConstraintModelImpl implements ConstraintModel {
		...
		private new(Collection<? extends ConstDecl<? extends Type>> constDecls,
			Collection<? extends Expr<? extends BoolType>> constraints) {
