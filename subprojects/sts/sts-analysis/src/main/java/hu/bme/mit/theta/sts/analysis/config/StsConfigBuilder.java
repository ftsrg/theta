...
public final class StsConfigBuilder {
    ...
    public enum InitPrec {
        EMPTY(new StsEmptyInitPrec()),
        PROP(new StsPropInitPrec());
    ...
    public StsConfig<? extends State, ? extends Action, ? extends Prec> build(final STS sts) {
        ...
        if (domain == Domain.EXPL) {
            ...
            final Analysis<ExplState, ExprAction, ExplPrec> analysis =
                    ExplAnalysis.create(analysisSolver, init);
            final ArgBuilder<ExplState, StsAction, ExplPrec> argBuilder =
                    ArgBuilder.create(lts, analysis, target, true);
            final Abstractor<ExplState, StsAction, ExplPrec> abstractor =
        ...
        } else if (domain == Domain.PRED_BOOL
            ...
            final Analysis<PredState, ExprAction, PredPrec> analysis =
                    PredAnalysis.create(analysisSolver, predAbstractor, init);
            final ArgBuilder<PredState, StsAction, PredPrec> argBuilder =
                    ArgBuilder.create(lts, analysis, target, true);
            final Abstractor<PredState, StsAction, PredPrec> abstractor =
                    BasicAbstractor.builder(argBuilder)
                            .waitlist(PriorityWaitlist.create(search.comparator))
                            .stopCriterion(
                                    refinement == Refinement.MULTI_SEQ
