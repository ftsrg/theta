...
fun XSTS.toMonolithicExprEnvTran(): MonolithicExpr {
...
fun XSTS.valToActionEnvTran(val1: Valuation, val2: Valuation): XstsAction {
    return XstsAction.create(
        Stmts.SequenceStmt(listOf(this.env, this.tran))
    )
}

fun XSTS.valToStateEnvTran(val1: Valuation): XstsState<ExplState> {
    return XstsState.of(
        ExplState.of(val1),
        false,
        true
