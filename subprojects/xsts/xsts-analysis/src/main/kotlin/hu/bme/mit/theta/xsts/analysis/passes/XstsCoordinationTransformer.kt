...
object XstsCoordinationTransformer {
  ...
  fun transform(xsts: XSTS): XSTS {
    ...
    for (i in 0 until tranSeq.stmts.size) {
      if (tranSeq.stmts[i] is IfStmt) break else delays.add(tranSeq.stmts[i])
    }
    ...
    tranSeq.stmts.filterIsInstance<IfStmt>().forEach {
      schedulings[(it.cond as EnumEqExpr).rightOp as EnumLitExpr] =
        SequenceStmt.of(listOf(delayStmt, it.then))
    }
    ...
    val branches =
      envChoice.stmts
        .filterIsInstance<SequenceStmt>()
        .map {
          SequenceStmt.of(
            it.stmts.map {
              if (
                it is AssignStmt<*> && it.expr is EnumLitExpr && schedulings.keys.contains(it.expr)
              ) {
                schedulings[it.expr]!!
              } else {
                it
