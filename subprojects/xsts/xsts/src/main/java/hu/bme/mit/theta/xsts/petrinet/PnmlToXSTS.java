...
import hu.bme.mit.theta.frontend.petrinet.pnml.elements.*;
...
public class PnmlToXSTS {
    ...
    public static XSTS createXSTS(final PnmlNet net, final InputStream propStream) {
		...
		for (PnmlPlace place: net.getPlaces()){
			...
			initExprs.add(Eq(placeVar.getRef(), IntExprs.Int(place.getInitialMarking())));
		}
        ...
        for (PnmlTransition transition : net.getTransitions()) {
			...
			for(PnmlArc inArc: transition.getInArcs()){
				...
				Preconditions.checkArgument(sourceNode instanceof PnmlPlace,"Transition %s has an illegal source",transition.getId(),sourceNode.getId());
			...
			for (PnmlArc outArc: transition.getOutArcs()){
				...
				Preconditions.checkArgument(targetNode instanceof PnmlPlace,"Transition %s has an illegal target %s",transition.getId(),targetNode.getId());
