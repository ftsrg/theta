...
public class XstsCli {
	...
	@Parameter(names = "--maxatomcount", description = "Number of different atoms a variable has to appear in to be switched to explicit tracking (0: unlimited)")
	Integer maxAtomCount = 0;
	...
	private XstsConfig<?, ?, ?> buildConfiguration(final XSTS xsts) throws Exception {
		try {
			return new XstsConfigBuilder(domain, refinement, Z3SolverFactory.getInstance())
					.maxEnum(maxEnum).maxAtomCount(maxAtomCount).initPrec(initPrec).pruneStrategy(pruneStrategy)
