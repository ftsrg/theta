...
The tool is used with the following by default:

```
INPUT_FLAG --model <input-file>
```

Here, the `<input-file>` must be a pre-transformed theta-flavored CFA created by [plcverif](https://gitlab.com/plcverif-oss).

For a more verbose output, change `--loglevel` to `MAINSTEP, SUBSTEP, INFO or DETAIL`.
For more parameters, check out [the documentation on CEGAR](https://github.com/ftsrg/theta/blob/master/doc/CEGAR-algorithms.md) *(might not be up to date on every parameter, but gives a detailed explanation on them)* and [the main class of the XSTS project](https://github.com/ftsrg/theta/blob/master/subprojects/xsts/xsts-cli/src/main/java/hu/bme/mit/theta/cfa/cli/XstsCli.java).
