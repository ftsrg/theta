...
class XstsCliTracegen :
    ...
    option(
        help =
        "If this flag is present, a concretized summary is generated."
    ).boolean().default(false)
    val traces: Boolean by
    option(
        help =
        "If this flag is present, concretized traces are generated."
    ).boolean().default(false)
    ...
    private fun toCexs(
        ...
        for ((node, state) in summaryStateMap) {
            sb.add(
                Utils.lispStringBuilder("_${node.id}: ${XstsState::class.java.simpleName}") // TODO number env separately
    ...
    private fun printResult(
        ...
        if (traces) {
            ...
            val concretizationResultTuple = XstsTraceGenerationConcretizerUtil.concretizeTraceList(
                traces as List<Trace<XstsState<ExplState>, XstsAction>>, Z3LegacySolverFactory.getInstance(), xsts
            )
            ...
            for (trace in concretizedTraces) {
                val traceFile = File(
                    traceDirPath.absolutePath + File.separator + Files.getNameWithoutExtension(
                        inputOptions.model.name
                    ) + "-" + traceCount + ".trace"
                )
                logger.write(Logger.Level.SUBSTEP, "Writing trace into file: %s%n", traceFile.path)
                PrintWriter(traceFile).use { printWriter ->
            ...
            val reportFile = File(
                traceDirPath.absolutePath + File.separator + "report.txt"
            )
            logger.write(Logger.Level.MAINSTEP, "Writing report into file: %s%n", reportFile.path)

            PrintWriter(reportFile).use { printWriter ->
