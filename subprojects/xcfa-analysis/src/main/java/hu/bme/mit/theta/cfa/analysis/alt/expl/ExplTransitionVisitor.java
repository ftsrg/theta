...
import hu.bme.mit.theta.cfa.analysis.alt.transform.InternalNotifyStmt;
import hu.bme.mit.theta.cfa.dsl.CallStmt;
import hu.bme.mit.theta.cfa.type.SyntheticLitExpr;
...
final class ExplTransitionVisitor implements XcfaStmtVisitor<ExplStateReadOnlyInterface, Optional<TransitionExecutorInterface>> {
    ...
    public Optional<TransitionExecutorInterface> visit(XcfaCallStmt stmt, ExplStateReadOnlyInterface readOnlyState) {
        return Optional.of(state -> StmtHelper.call(state, readOnlyState.getTransitionProcess(), (CallStmt)stmt));
    }
