...
- [stmt/](src/main/java/hu/bme/mit/theta/core/stmt/) — Stmt taxonomy: atomic `Skip`/`Assume`/`Assign`/`MemoryAssign`/`Havoc` + composite `Sequence`/`NonDet`/`Ort`/`Loop`/`If`. Only the atomics (minus MemoryAssign) are reachable from any text format; composites are Java-only. `NonDet`/`Ort`/`Sequence` silently coerce an empty list to `[Skip]`.
