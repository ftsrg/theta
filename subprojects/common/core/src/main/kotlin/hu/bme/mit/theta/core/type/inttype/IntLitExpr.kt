...
@Serializable
@SerialName("IntLit")
data class IntLitExpr(@Serializable(with = BigIntegerSerializer::class) val value: BigInteger) :
  NullaryExpr<IntType>(), LitExpr<IntType>, Comparable<IntLitExpr> {

  companion object {

    @JvmStatic fun of(value: BigInteger) = IntLitExpr(value)
  }
  ...
  fun rem(that: IntLitExpr): IntLitExpr {
    ...
    return when {
      ...
      this.value >= BigInteger.ZERO && that.value < BigInteger.ZERO ->
        IntLitExpr(thisAbs.mod(thatAbs).negate())
      this.value < BigInteger.ZERO && that.value >= BigInteger.ZERO -> {
        var result = thisAbs.mod(thatAbs)
