...
data class RatLitExpr(
  @Serializable(with = BigIntegerSerializer::class) val num: BigInteger,
  @Serializable(with = BigIntegerSerializer::class) val denom: BigInteger,
  ...
  companion object {
    ...
    fun of(num: BigInteger, denom: BigInteger): RatLitExpr {
      ...
      val (simplifiedNum, simplifiedDenom) =
  ...
  fun mul(that: RatLitExpr) = of(this.num * that.num, this.denom * that.denom)

  fun div(that: RatLitExpr) = of(this.num * that.denom, this.denom * that.num)
