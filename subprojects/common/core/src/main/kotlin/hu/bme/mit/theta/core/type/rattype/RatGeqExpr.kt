...
@Serializable
@SerialName("RatGeq")
data class RatGeqExpr(override val leftOp: Expr<RatType>, override val rightOp: Expr<RatType>) :
  GeqExpr<RatType>() {

  companion object {

    @JvmStatic fun of(leftOp: Expr<RatType>, rightOp: Expr<RatType>) = RatGeqExpr(leftOp, rightOp)

    @JvmStatic
    fun create(leftOp: Expr<*>, rightOp: Expr<*>) =
      RatGeqExpr(cast(leftOp, Rat()), cast(rightOp, Rat()))
  }
  ...
  override fun new(leftOp: Expr<RatType>, rightOp: Expr<RatType>): RatGeqExpr = of(leftOp, rightOp)
