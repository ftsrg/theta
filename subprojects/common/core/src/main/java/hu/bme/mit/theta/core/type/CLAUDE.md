...
6. Pick a **unique** `OPERATOR_LABEL` (there are existing collisions — see below — don't add more) and a fresh `HASH_SEED`.
...
## Known label collisions (don't rely on toString round-trips)

- `"="` — `FpEqExpr` and `FpAssignExpr`
- `"bvpos"` — `BvPosExpr` and `BvSignChangeExpr`
