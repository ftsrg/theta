...
data class ArrayReadExpr<IndexType : Type, ElemType : Type>(
    ...
    override fun eval(`val`: Valuation): LitExpr<ElemType> {
        ...
        arrayVal.elements.forEach { elem ->
            if (elem.first == indexVal) return elem.second
        }
    ...
    fun with(
        array: Expr<ArrayType<IndexType, ElemType>>, index: Expr<IndexType>
    ): ArrayReadExpr<IndexType, ElemType> =
    ...
    fun withArray(
        array: Expr<ArrayType<IndexType, ElemType>>
    ): ArrayReadExpr<IndexType, ElemType> = with(array, index)
