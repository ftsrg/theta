...
import hu.bme.mit.theta.core.type.clocktype.ClockGtExpr;
...
public final class DiffGtConstr extends DiffConstr {
	...
	private volatile ClockGtExpr expr = null;

	DiffGtConstr(final VarDecl<ClockType> leftVar, final VarDecl<ClockType> rightVar, final int bound) {
		super(leftVar, rightVar, bound);
	}

	@Override
	public ClockGtExpr toExpr() {
		ClockGtExpr result = expr;
		if (result == null) {
			...
			result = ClockExprs.Gt(diffExpr, Int(getBound()));
			expr = result;
		}
		return result;
	}
