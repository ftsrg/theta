...
public class BvType implements Additive<BvType>, Multiplicative<BvType>, Divisible<BvType>, Equational<BvType>, Ordered<BvType>,
        Castable<BvType> {
    ...
    public RemExpr<BvType> Rem(Expr<BvType> leftOp, Expr<BvType> rightOp) {
        checkState(signed != null, "Neutral BvType cannot be used here");
        if(signed) return BvExprs.SRem(leftOp, rightOp);
        else return BvExprs.URem(leftOp, rightOp);
    }
    ...
    public DivExpr<BvType> Div(Expr<BvType> leftOp, Expr<BvType> rightOp) {
        checkState(signed != null, "Neutral BvType cannot be used here");
        if(signed) return BvExprs.SDiv(leftOp, rightOp);
        else return BvExprs.UDiv(leftOp, rightOp);
    }

    @Override
    public LtExpr<BvType> Lt(Expr<BvType> leftOp, Expr<BvType> rightOp) {
        checkState(signed != null, "Neutral BvType cannot be used here");
        if(signed) return BvExprs.SLt(leftOp, rightOp);
        else return BvExprs.ULt(leftOp, rightOp);
    }

    @Override
    public LeqExpr<BvType> Leq(Expr<BvType> leftOp, Expr<BvType> rightOp) {
        checkState(signed != null, "Neutral BvType cannot be used here");
        if(signed) return BvExprs.SLeq(leftOp, rightOp);
        else return BvExprs.ULeq(leftOp, rightOp);
    }

    @Override
    public GtExpr<BvType> Gt(Expr<BvType> leftOp, Expr<BvType> rightOp) {
        checkState(signed != null, "Neutral BvType cannot be used here");
        if(signed) return BvExprs.SGt(leftOp, rightOp);
        else return BvExprs.UGt(leftOp, rightOp);
    }

    @Override
    public GeqExpr<BvType> Geq(Expr<BvType> leftOp, Expr<BvType> rightOp) {
        checkState(signed != null, "Neutral BvType cannot be used here");
        if(signed) return BvExprs.SGeq(leftOp, rightOp);
        else return BvExprs.UGeq(leftOp, rightOp);
    }
}
