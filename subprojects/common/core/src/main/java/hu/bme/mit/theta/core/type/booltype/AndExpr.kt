...
@Serializable
@SerialName("And")
data class AndExpr(override val ops: List<Expr<BoolType>>) : MultiaryExpr<BoolType, BoolType>() {

  companion object {

    private const val OPERATOR_LABEL = "and"

    @JvmStatic fun of(ops: Iterable<Expr<BoolType>>) = AndExpr(ops.toList())

    @JvmStatic fun create(ops: List<Expr<*>>) = AndExpr(ops.map { cast(it, Bool()) })
  }
  ...
  override fun new(ops: List<Expr<BoolType>>): AndExpr = of(ops)
