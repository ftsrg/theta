...
import hu.bme.mit.theta.core.type.clocktype.ClockLtExpr;
...
public final class DiffLtConstr extends DiffConstr {
	...
	private volatile ClockLtExpr expr = null;

	DiffLtConstr(final VarDecl<ClockType> leftVar, final VarDecl<ClockType> rightVar, final int bound) {
		super(leftVar, rightVar, bound);
	}

	@Override
	public ClockLtExpr toExpr() {
		ClockLtExpr result = expr;
		if (result == null) {
			...
			result = ClockExprs.Lt(diffExpr, Int(getBound()));
			expr = result;
		}
		return result;
	}
