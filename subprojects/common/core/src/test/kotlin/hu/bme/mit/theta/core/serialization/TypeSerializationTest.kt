...
class TypeSerializationTest {
    ...
    companion object {
        ...
        private val json = Json {
            serializersModule = typeSerializerModule
            classDiscriminator = "type"
        }

        @JvmStatic
        val bvTypeTestData: List<Arguments> = listOf(
    ...
    fun `test ArrayType serialization`() {
        ...
        val complex: Type = ArrayType(
            indexType = BvType(8, false),
            elemType = ArrayType(
                indexType = ArrayType(IntType, RatType),
                elemType = BoolType,
            )
        )
    ...
    fun `test specific serializer must ignore type field`() {
        ...
        val serialized = json.encodeToString(original)
        assertDoesNotThrow {
            (json.decodeFromString<Type>(serialized) as BvType).size
        }
        assertThrows<SerializationException> {
            json.decodeFromString<BvType>(serialized).size
        }
        ...
        assertDoesNotThrow {
            jsonIgnoreUnknownKey.decodeFromString<BvType>(serialized).size
        }
    }
}
