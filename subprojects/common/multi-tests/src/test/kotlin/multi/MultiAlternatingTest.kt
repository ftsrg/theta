...
class MultiAlternatingTest {
  ...
  fun testExplPrec() {
    ...
    try {
      SequenceInputStream(
        ...
        .use { inputStream -> xsts = XstsDslManager.createXsts(inputStream) }
    } catch (e: IOException) {
      throw RuntimeException(e)
    }

    val xstsConfigBuilder =
      XstsConfigBuilder(
    ...
    val product =
      StmtMultiBuilder(cfaExplBuilder.multiSide, cfaExplBuilder.lts)
    ...
    val multiConfigBuilder =
      StmtMultiConfigBuilder.ItpStmtMultiConfigBuilder(
        product,
        prop,
  ...
  fun testPredPrec() {
    ...
    val cfaRefToPrec =
      RefutationToGlobalCfaPrec(ItpRefToPredPrec(ExprSplitters.atoms()), cfa.initLoc)
    val dataInitPrec = PredPrec.of()
    val cfaInitPrec: CfaPrec<PredPrec> = GlobalCfaPrec.create(dataInitPrec)

    val product =
      StmtMultiBuilder(cfaPredBuilder.multiSide, cfaPredBuilder.lts)
