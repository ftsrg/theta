  ...
  private class ExprCreatorVisitor(scope: Scope, env: Env) : ExprBaseVisitor<Expr<out Type>>() {
    ...
    override fun visitFuncLitExpr(ctx: FuncLitExprContext): Expr<out Type> {
      return if (ctx.result != null) {
        val param =
          Decls.Param(ctx.param.name.text, TypeWrapper(ctx.param.type().textWithWS()).instantiate())
        push(listOf(param))
    ...
    private fun createParamList(ctx: DeclListContext): List<ParamDecl<*>> {
      return if (ctx.decls == null) {
        ...
        ctx.decls
          .stream()
          .map { d: DeclContext ->
            Decls.Param(d.name.text, TypeWrapper(d.ttype.textWithWS()).instantiate())
          }
    ...
    override fun visitIteExpr(ctx: IteExprContext): Expr<out Type> {
      return if (ctx.cond != null) {
        val cond: Expr<BoolType> = cast(ctx.cond.accept<Expr<*>>(this), BoolExprs.Bool())
    ...
    override fun visitForallExpr(ctx: ForallExprContext): Expr<out Type> {
      return if (ctx.paramDecls != null) {
        ...
        push(paramDecls)
        val op: Expr<BoolType> = cast(ctx.op.accept<Expr<*>>(this), BoolExprs.Bool())
        pop()
    ...
    override fun visitBitwiseOrExpr(ctx: BitwiseOrExprContext): Expr<out Type> {
      return if (ctx.oper != null) {
        val opStream: Stream<Expr<(BvType)>> =
          ctx.ops.stream().map { op: ExprContext -> op.accept<Expr<*>>(this) as Expr<BvType> }
    ...
    override fun visitBitwiseXorExpr(ctx: BitwiseXorExprContext): Expr<out Type> {
      return if (ctx.rightOp != null) {
        ...
        when (ctx.oper.type) {
          BV_XOR -> BvExprs.Xor(listOf(leftOp, rightOp))
          else -> throw ParseException(ctx, "Unknown operator")
        }
      } else {
        visitChildren(ctx)
    ...
    override fun visitAdditiveExpr(ctx: AdditiveExprContext): Expr<out Type> {
      return if (ctx.ops.size >= 1) {
        val opStream: Stream<Expr<*>> =
          ctx.ops.stream().map(Function { op: ExprContext -> op.accept<Expr<*>>(this) })
      ...
      return when (oper.type) {
        ...
        FPADD ->
          FpExprs.Add(
            ...
            listOf(TypeUtils.castFp(leftOp), TypeUtils.castFp(rightOp)),
          )

        FPSUB ->
          FpExprs.Sub(
            getRoundingMode(ctx.oper.text),
            TypeUtils.castFp(leftOp),
    ...
    private fun createAddExpr(leftOp: Expr<*>, rightOp: Expr<*>): AddExpr<*> {
      return if (leftOp is AddExpr<*>) {
        val ops: List<Expr<*>> =
          ImmutableList.builder<Expr<*>>().addAll(leftOp.ops).add(rightOp).build()
    ...
    private fun createBvAddExpr(leftOp: Expr<BvType>, rightOp: Expr<BvType>): BvAddExpr {
      return if (leftOp is BvAddExpr) {
        val ops: List<Expr<BvType>> =
          ImmutableList.builder<Expr<BvType>>().addAll(leftOp.ops).add(rightOp).build()
        BvExprs.Add(ops)
      } else {
        BvExprs.Add(listOf(leftOp, rightOp))
      }
    }
    ...
    private fun createMultiplicativeSubExpr(
      ...
      return when (oper.type) {
        ...
        FPMUL ->
          FpExprs.Mul(
            getRoundingMode(ctx.oper.text),
            listOf(leftOp as Expr<FpType>, rightOp as Expr<FpType>),
          )

        FPDIV ->
          FpExprs.Div(
            getRoundingMode(ctx.oper.text),
            leftOp as Expr<FpType>,
    ...
    private fun createBvMulExpr(leftOp: Expr<BvType>, rightOp: Expr<BvType>): BvMulExpr {
      return if (leftOp is BvMulExpr) {
        ...
        BvExprs.Mul(listOf(leftOp, rightOp))
      }
    }
    ...
    private fun createBvConcatExpr(leftOp: Expr<BvType>, rightOp: Expr<BvType>): BvConcatExpr {
      return if (leftOp is BvConcatExpr) {
        ...
        BvExprs.Concat(listOf(leftOp, rightOp))
      }
    }

    override fun visitBvExtendExpr(ctx: BvExtendExprContext): Expr<out Type> {
      return if (ctx.rightOp != null) {
        val extendType = BvExprs.BvType(ctx.rightOp.size.text.toInt())
        when (ctx.oper.type) {
          BV_ZERO_EXTEND ->
            BvExprs.ZExt(TypeUtils.castBv(ctx.leftOp.accept<Expr<*>>(this)), extendType)

          BV_SIGN_EXTEND ->
            BvExprs.SExt(TypeUtils.castBv(ctx.leftOp.accept<Expr<*>>(this)), extendType)
    ...
    override fun visitUnaryExpr(ctx: UnaryExprContext): Expr<out Type> {
      return if (ctx.op != null) {
        ...
        when (ctx.oper.type) {
          ...
          FPROUNDTOINT ->
            FpExprs.RoundToIntegral(getRoundingMode(ctx.oper.text), op as Expr<FpType>)
          ...
          FPTOFP ->
            FpExprs.ToFp(
              getRoundingMode(ctx.oper.text),
              op as Expr<FpType>,
              getExp(ctx.oper.text),
              getSignificand(ctx.oper.text),
            )
          ...
          FP_FROM_BV ->
            FpExprs.FromBv(
              getRoundingMode(ctx.oper.text),
              op as Expr<BvType>,
              FpType.of(getExp(ctx.oper.text), getSignificand(ctx.oper.text)),
              isSignedFp(ctx.oper.text),
            )
    ...
    override fun visitArrayRead(ctx: ArrayReadContext): Expr<out Type> {
      return if (ctx.array != null) {
        ArrayReadExpr.create<Type, Type>(ctx.array.accept(this), ctx.index.accept(this))
      } else {
        visitChildren(ctx)
      }
    }
    ...
    override fun visitBvExtract(ctx: BvExtractContext): Expr<out Type> {
      return if (ctx.op != null) {
        ...
        return BvExprs.Extract(bitvec, Int(ctx.from.text), Int(ctx.until.text))
      } else {
        visitChildren(ctx)
      }
    }
    ...
    override fun visitArrLitExpr(ctx: ArrLitExprContext): Expr<out Type> {
      ...
      val elems =
        ctx.indexExpr.mapIndexed { idx, it ->
          Pair(it.accept(this), ctx.valueExpr[idx].accept(this))
        }
      return ExprUtils.simplify(
        ArrayInitExpr.create<Type, Type>(elems, elseElem, ArrayType.of(indexType, valueType))
      )
    }

    override fun visitBvLitExpr(ctx: BvLitExprContext): Expr<out Type> {
      val sizeAndContent: Array<String> =
        ctx.bv.text.split("['#]".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
    ...
    override fun visitIdExpr(ctx: IdExprContext): RefExpr<*> {
      val optSymbol = currentScope.resolve(ctx.id.text)
