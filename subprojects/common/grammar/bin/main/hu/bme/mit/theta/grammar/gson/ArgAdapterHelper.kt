...
data class ArgAdapterHelper<S : State, A : Action>(
  ...
  constructor(
    ...
    initNodes =
      arg.initNodes
        .map { Pair(it.id, Triple(it.state, it.isTarget, it.isExpanded)) }
        .toList()
        .associate { it.first to it.second },
    nodes =
      arg.nodes
        .map { Pair(it.id, ArgNodeAdapter(it.isTarget, it.state, it.isExpanded)) }
        .toList()
        .associate { it.first to it.second },
    edges =
      arg.nodes
        .filter { it.inEdge.isPresent }
        .map { Pair(it.id, ArgEdgeAdapter(it.inEdge.get().source.id, it.inEdge.get().action)) }
        .toList()
        .associate { it.first to it.second },
    coveringEdges =
      arg.nodes
        .filter { it.coveringNode.isPresent }
        .map { Pair(it.id, it.coveringNode.get().id) }
        .toList()
        .associate { it.first to it.second },
  )
  ...
  fun instantiate(partialOrd: PartialOrd<S>): ARG<S, A> {
    ...
    initNodes.forEach {
      lut[it.key] =
        arg.createInitNode(it.value.first, it.value.second).also { n ->
          if (it.value.third) n.expanded = true
        }
    }
      ...
      lut[entry] =
        arg
          .createSuccNode(
            lut[edge.source],
            edge.action,
            checkNotNull(nodes[entry]).state,
...
data class ArgNodeAdapter<S : State>(val target: Boolean, val state: S, val expanded: Boolean)

data class ArgEdgeAdapter<A : Action>(val source: Int, val action: A) {}
