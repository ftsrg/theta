...
class MultiAnalysisTest {
  ...
  val multiUnitPrec =
    MultiPrec(UnitPrec.getInstance(), UnitPrec.getInstance(), UnitPrec.getInstance())
  ...
  fun test() {
    ...
    val builderResult =
      StmtMultiBuilder(side, NumberedLTS())
        .addRightSide(side, NumberedLTS())
        .build<UnitPrec>(
          { ms: ExprMultiState<NumberedState, NumberedState, UnitState> ->
    ...
    val succ1State =
      multiAnalysis.transFunc.getSuccStates(initState, action, multiUnitPrec).single()
    ...
    val succ2State =
      multiAnalysis.transFunc.getSuccStates(succ1State, action, multiUnitPrec).single()
    ...
    val succ3State =
      multiAnalysis.transFunc.getSuccStates(succ2State, action, multiUnitPrec).single()
