...
public class MddConstrainedCursorTest {
    ...
    public static Collection<Object[]> data() {
        return Arrays.asList(new Object[][]{
                ...
                {List.of(X, Y),
                        BoolExprs.And(Eq(X.getRef(), Int(0)), Eq(Y.getRef(), Int(0))), // x = 0, y = 0
                        BoolExprs.And(Eq(Prime(X.getRef()), X.getRef()), Eq(Prime(Y.getRef()), Y.getRef())), // x'=x, y'=y
                        1},

                {List.of(X, Y),
                        BoolExprs.And(Eq(X.getRef(), Int(0)), Eq(Y.getRef(), Int(0))), // x = 0, y = 0
                        BoolExprs.And(Eq(Prime(X.getRef()), Add(X.getRef(), Int(1))), Eq(Prime(Y.getRef()), Y.getRef())), // x'=x + 1, y'=y
                        1},

                {List.of(X, Y),
                        Or(BoolExprs.And(Eq(X.getRef(), Int(0)), Eq(Y.getRef(), Int(0))), BoolExprs.And(Eq(X.getRef(), Int(1)), Eq(Y.getRef(), Int(1)))), // x = 0, y = 0 or x = 1, y = 1
                        BoolExprs.And(Eq(Prime(X.getRef()), X.getRef()), Eq(Prime(Y.getRef()), Y.getRef())), // x'=x, y'=y
                        2},

                {List.of(X, Y),
                        BoolExprs.And(Eq(X.getRef(), Int(0)), Eq(Y.getRef(), Int(0))), // x = 0, y = 0
                        BoolExprs.And(Eq(Prime(X.getRef()), Y.getRef()), Eq(Prime(Y.getRef()), Y.getRef())), // x'=y, y'=y
                        1},

                {List.of(X, Y),
                        BoolExprs.And(Eq(X.getRef(), Int(0)), Eq(Y.getRef(), Int(0))), // x = 0, y = 0
                        BoolExprs.And(Eq(Prime(X.getRef()), Add(Y.getRef(), Int(1))), Eq(Prime(Y.getRef()), Y.getRef())), // x'=y + 1, y'=y
                        1},
    ...
    public void test() throws Exception {
        ...
        try (final SolverPool solverPool = new SolverPool(Z3LegacySolverFactory.getInstance())) {
            ...
            final MddHandle constraintHandle = stateSig.getTopVariableHandle().checkInNode(MddExpressionTemplate.of(constraintUnfolded, o -> (Decl) o, solverPool));
            ...
            final var stateSpaceInfo = new MddStateSpaceInfo(stateSig.getTopVariableHandle().getVariable().orElseThrow(), constraintHandle.getNode());
