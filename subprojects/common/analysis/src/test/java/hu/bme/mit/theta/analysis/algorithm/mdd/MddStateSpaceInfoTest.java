...
public class MddStateSpaceInfoTest {
    ...
    public static Collection<Object[]> data() {
        return Arrays.asList(new Object[][]{
                ...
                {List.of(X, Y),
                        Or(And(Eq(X.getRef(), Int(0)), Eq(Y.getRef(), Int(0))), And(Eq(X.getRef(), Int(1)), Eq(Y.getRef(), Int(1))), And(Eq(X.getRef(), Int(2)), Eq(Y.getRef(), Int(2)))), // x = 0, y = 0 or x = 1, y = 1 or x = 2, y = 3
                        9L},
    ...
    public void test() throws Exception {
        ...
        try (final SolverPool solverPool = new SolverPool(Z3LegacySolverFactory.getInstance())) {
            ...
            final MddVariableOrder variableOrder = JavaMddFactory.getDefault().createMddVariableOrder(mddGraph);
            varOrder.forEach(v -> {
                final var domainSize = Math.max(v.getType().getDomainSize().getFiniteSize().intValue(), 0);
                variableOrder.createOnTop(MddVariableDescriptor.create(v.getConstDecl(0), domainSize));
            });
            final var signature = variableOrder.getDefaultSetSignature();

            final var stateUnfolded = PathUtils.unfold(stateSpaceExpr, 0);
            final MddHandle stateHandle = signature.getTopVariableHandle().checkInNode(MddExpressionTemplate.of(stateUnfolded, o -> (Decl) o, solverPool));

            final var stateSpaceInfo = new MddStateSpaceInfo(signature.getTopVariableHandle().getVariable().orElseThrow(), stateHandle.getNode());
            final var structuralRepresentation = stateSpaceInfo.toStructuralRepresentation();
            final var structuralHandle = signature.getTopVariableHandle().getHandleFor(structuralRepresentation);
