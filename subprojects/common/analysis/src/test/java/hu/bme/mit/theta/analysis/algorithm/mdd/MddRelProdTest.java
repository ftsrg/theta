...
public class MddRelProdTest {
    ...
    public static Collection<Object[]> data() {
        return Arrays.asList(new Object[][]{
                ...
                {List.of(X, Y),
                        ...
                        And(Eq(Prime(X.getRef()), X.getRef()), Eq(Prime(Y.getRef()), Y.getRef())), // x'=x, y'=y
                        1L},

                {List.of(X, Y),
                        And(Eq(X.getRef(), Int(0)), Eq(Y.getRef(), Int(0))), // x = 0, y = 0
                        And(Eq(Prime(X.getRef()), Add(X.getRef(), Int(1))), Eq(Prime(Y.getRef()), Y.getRef())), // x'=x + 1, y'=y
                        1L},

                {List.of(X, Y),
                        Or(And(Eq(X.getRef(), Int(0)), Eq(Y.getRef(), Int(0))), And(Eq(X.getRef(), Int(1)), Eq(Y.getRef(), Int(1)))), // x = 0, y = 0 or x = 1, y = 1
                        And(Eq(Prime(X.getRef()), X.getRef()), Eq(Prime(Y.getRef()), Y.getRef())), // x'=x, y'=y
                        2L},
                ...
                {List.of(A, B),
                        And(A.getRef(), B.getRef()),
                        And(Eq(B.getRef(), Prime(A.getRef())), Eq(B.getRef(), Prime(B.getRef()))), // a'=b, b'=b
                        1L},
                ...
                {List.of(A, C),
                        ...
                        And(Eq(A.getRef(), Prime(A.getRef())), Eq(C.getRef(), Prime(C.getRef()))), // a'=a, c'=c
                        1L},
                ...
                {List.of(A, C),
                        And(A.getRef(), Eq(C.getRef(), RED)),
                        And(Eq(A.getRef(), Prime(A.getRef())), Or(Eq(Prime(C.getRef()), RED), Eq(Prime(C.getRef()), GREEN), Eq(Prime(C.getRef()), BLUE))), // a'=a
                        3L},
    ...
    public void test() throws Exception {
        ...
        try (final SolverPool solverPool = new SolverPool(Z3LegacySolverFactory.getInstance())) {
            ...
            final MddHandle stateHandle = stateSig.getTopVariableHandle().checkInNode(MddExpressionTemplate.of(stateUnfolded, o -> (Decl) o, solverPool));
            final MddHandle transHandle = transSig.getTopVariableHandle().checkInNode(MddExpressionTemplate.of(transUnfolded, o -> (Decl) o, solverPool));

            final AbstractNextStateDescriptor nextStateDescriptor = MddNodeNextStateDescriptor.of(transHandle);

            final var provider = new LegacyRelationalProductProvider(stateSig.getVariableOrder());
            final var result = provider.compute(stateHandle, nextStateDescriptor, stateSig.getTopVariableHandle());
