 ...
 * reachable. The bound is a relation: it permits a transition only if its source was reached and
 * the source→target step is SAT — and since a SAT step from a reached source lands in the reach
 * set, `bound ⊆ reach`. So the bound additionally prunes steps that are infeasible from their
 * (reached) source yet land in a reachable target, which the target-only postcondition cannot
 * express. The coupling `y' ∈ [y, y+x]` creates exactly such steps (from high `y` a lower `y` is
 * unreachable since `y' ≥ y`, yet that lower `y` is reachable from a smaller-`y` state). Both agree
 * on the verdict (the bound is a sound replacement for the reach constraint), but the bound issues
 * strictly fewer checks.
 */
