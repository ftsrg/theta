...
public class MddNodeNextStateDescriptor implements AbstractNextStateDescriptor {
    ...
    private static AbstractNextStateDescriptor of(MddNode node, MddVariableHandle variableHandle) {
        if (node == null || node == variableHandle.getMddGraph().getTerminalZeroNode()) return AbstractNextStateDescriptor.terminalEmpty();
        if (node.getRepresentation() instanceof IdentityExpressionRepresentation identityExpressionRepresentation){
            final var cont = identityExpressionRepresentation.getContinuation();
            return IdentityNextStateDescriptor.withChild(MddNodeNextStateDescriptor.of(cont, variableHandle.getLower().orElseThrow().getLower().orElseThrow()));
        }
