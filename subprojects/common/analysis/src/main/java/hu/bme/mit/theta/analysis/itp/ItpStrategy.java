...
public abstract class ItpStrategy<SConcr extends State, SAbstr extends ExprState, SItp extends State, S extends State, A extends Action, P extends Prec>
        implements AlgorithmStrategy<SConcr, SAbstr, S, A> {

    protected final Lens<S, ItpState<SConcr, SAbstr>> lens;
    ...
    protected ItpStrategy(final Lens<S, ItpState<SConcr, SAbstr>> lens,
    ...
    @Override
    public final void block(final ArgNode<S, A> node, final A action, final S succState, final Collection<ArgNode<S, A>> uncoveredNodes){
    ...
    protected final void strengthen(final ArgNode<S, A> node, final SAbstr interpolant){
        ...
        final ItpState<SConcr, SAbstr> itpState = lens.get(state);
        final SAbstr abstrState = itpState.getAbstrState();
        final SAbstr newAbstrState = abstrLattice.meet(abstrState, interpolant);
        final ItpState<SConcr, SAbstr> newItpState = itpState.withAbstrState(newAbstrState);
        final S newState = lens.set(state, newItpState);
