...
class TraceGenerationChecker<S : ExprState, A : ExprAction, P : Prec> constructor(
    logger: Logger,
    ...
    var traces: List<Trace<S, A>> = ArrayList()
        private set
    ...
    override fun check(prec: P): SafetyResult<S, A> {
        ...
        // TODO write down how does it work and why did we have full traces exactly
        // it was something with e.g., not exploring the whole lassos/loops in some cases, if there was a coverage on that branch
        // and I think this just goes as long as it can...? more precisely..?
        ...
        Preconditions.checkState(
            traces.size > 0,
            "Generated 0 traces, variable configuration is probably insufficient"
        )
        return SafetyResult.unsafe(
            traces[0],
            ARG.create { state1: S, state2: S -> false }) // this is only a placeholder
    }
    ...
    private fun computeCrossCoveredEndNodes(filteredEndNodes: List<ArgNode<S, A>>): List<ArgNode<S, A>> {
