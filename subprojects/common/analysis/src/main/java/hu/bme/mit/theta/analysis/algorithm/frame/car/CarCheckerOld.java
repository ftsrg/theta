...
import hu.bme.mit.theta.analysis.algorithm.frame.base.Clause;
...
public class CarCheckerOld<S extends ExprState, A extends ExprAction>
    ...
    public CarCheckerOld(
            MonolithicExpr monolithicExpr, SolverFactory solverFactory, Logger logger) {
        this(
                monolithicExpr,
                solverFactory,
    ...
    public SafetyResult<PredState, Trace<ExplState, ExprAction>> check(UnitPrec prec) {
        ...
        while (true) {
            ...
            if (propagateResult > 0) {
                return SafetyResult.safe(PredState.of(frames.get(propagateResult).getExpression()));
            }
        }
    }
            ...
            try (var wpp = new WithPushPop(solver)) {
                ...
                if (solver.check().isSat()) {
                    ...
                    if (optimizations.isFilterOpt()) {
                        var vars = CollectionUtil.createSet(filteredModel.toMap().keySet());
                        for (var var : vars) {
    ...
    public Node checkCurrentFrame(Node target) {
        if (optimizations.isPropertyOpt()) {
            var interSection =
                    convertValuationToExpression(
                            frames.get(currentFrameNumber)
                                    .checkIfTargetIsReachableValuation(And(target.getExprs())));
            try (var wpp = new WithPushPop(solver)) {
            ...
            var counterExample =
                    convertValuationToExpression(
                            frames.get(currentFrameNumber)
                                    .checkIfContainsValuation(And(target.getExprs())));
            if (counterExample == null) {
                return null; // no intersection found
            } else {
                Node newNode =
                        new Node(And(counterExample), target, optimizations.isCoverOpt(), solver);
    ...
    public int propagate() {
        ...
        if (frames.size() <= currentFrameNumber + 1) { // todo if needed?
            frames.add(
                    new Frame(
                            frames.get(currentFrameNumber), solver, monolithicExpr, optimizations));
        }
        currentFrameNumber++;
