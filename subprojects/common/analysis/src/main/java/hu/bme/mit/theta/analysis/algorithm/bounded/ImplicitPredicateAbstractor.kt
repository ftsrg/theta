...
/**
 * Implicit predicate abstraction over a [MonolithicExpr]. The abstract relation is a disjunction of
 * groups of concrete transitions with the same connected literals (predicates sharing a variable
 * with the transition, transitively); only those carry their definitions, the rest are kept by v' =
 * v.
 */
...
constructor(
  ...
  // abstract disjunct index -> indices of the concrete transitions in it, for the last
  // abstractModel
  ...
  fun abstractModel(prec: PredPrec): AbstractionResult {
    ...
    // transOffsetIndex: default offset 1 (literals, ctrl vars assigned once); non-ctrl concrete
    // vars
    ...
    // events over the connected closure: the literal levels an abstract transition node really
    // spans
   ...
   * concrete model's actions, under the prec of the last [abstractModel] call. A step whose
   * abstract action names the fired disjunct gets the concrete transitions of that group that are
   * consistent with the control valuations of the two abstract states (usually exactly one); any
   * other step gets the whole concrete relation.
   */
  ...
  /**
   * The literals connected to a transition touching [transitionVars]: those whose predicate shares
   * a variable with the transition, closed under sharing a variable with a connected predicate.
   */
  ...
  /**
   * The variables a transition reads or writes: all variables of its non-frame conjuncts, plus
   * every variable the relation may change (offset above 0) that this transition does not frame — a
