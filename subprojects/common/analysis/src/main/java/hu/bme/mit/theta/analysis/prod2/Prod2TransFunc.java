...
final class Prod2TransFunc<
    ...
    @Override
    public Collection<Prod2State<S1, S2>> getSuccStates(
            final Prod2State<S1, S2> state, final A action, final Prod2Prec<P1, P2> prec) {
        checkNotNull(state);
        checkNotNull(action);
        checkNotNull(prec);
        ...
        final Collection<Prod2State<S1, S2>> succStates =
                Prod2State.cartesian(succStates1, succStates2);
