...
public class NewAtomsAutoExpl implements AutoExpl {
    ...
    public NewAtomsAutoExpl(
            final Set<VarDecl<?>> explPreferredVars,
            final Set<Expr<BoolType>> modelAtoms,
            final int newAtomsLimit) {
    ...
    public void update(final Expr<BoolType> itp) {
        ...
        explVars.addAll(
                ExprUtils.getVars(itp).stream()
                        .filter(
                                decl ->
                                        newAtoms.containsKey(decl)
                                                        && newAtoms.get(decl).size() > newAtomsLimit
                                                || decl.getType() == Bool())
                        .collect(Collectors.toSet()));
    }
}
