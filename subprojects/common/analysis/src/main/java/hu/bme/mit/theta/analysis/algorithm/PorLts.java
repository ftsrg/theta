...
public abstract class PorLts<S extends State, A extends Action, T> implements LTS<S, A> {
    ...
    /**
     * Shared objects (~global variables) that are used by the key transition or by transitions
     * reachable from the current state via a given transition.
     */
    ...
    public Collection<A> getEnabledActionsFor(S state) {
        ...
        for (A firstAction : persistentSetFirstActions) {
            ...
            if (minimalPersistentSet.size() == 0
                || persistentSet.size() < minimalPersistentSet.size()) {
    ...
    protected Collection<A> calculatePersistentSet(Collection<A> enabledActions, A firstAction) {
        ...
        while (addedNewAction) {
            ...
            for (A action : otherActions) {
                ...
                if (persistentSet.stream()
                    .anyMatch(persistentSetAction -> areDependents(persistentSetAction, action))) {
                    if (isBackwardAction(action)) {
                        return new HashSet<>(
                            enabledActions); // see POR algorithm for the reason of removing backward transitions
                    }
    ...
    /**
     * Returns the actions from where persistent sets will be calculated (a subset of the given
     * enabled actions). The default implementation returns all enabled actions.
     *
    ...
    /**
     * Determines whether an action is dependent with another one (based on the notions introduced
     * for the POR algorithm) already in the persistent set.
     *
    ...
    /**
     * Determines whether two actions can enable or disable each other (if true, the two actions are
     * dependent).
     *
    ...
    /**
     * Returns the successive transitions of a transition (transitions whose source is the target of
     * the given parameter).
     *
    ...
    /**
     * Returns the shared objects (~global variables) that an action uses (it is present in one of
     * its labels).
     *
     * @param transition whose shared objects are to be returned
     * @return the set of used shared objects
     */
    protected abstract Set<? extends Decl<? extends Type>> getDirectlyUsedSharedObjects(
        T transition);
    ...
    /**
     * Returns the shared objects (~global variables) used by the given transition or by transitions
     * that are reachable via the given transition ("influenced shared objects").
     *
    ...
    /**
     * Returns shared objects (~global variables) encountered in a search starting from a given
     * transition.
     *
     ...
     */
    protected Set<? extends Decl<? extends Type>> getSharedObjectsWithBFS(T startTransition,
        Predicate<T> visitTransition) {
        ...
        while (!transitionsToExplore.isEmpty()) {
            ...
            for (var newTransition : successiveTransitions) {
                if (!exploredTransitions.contains(newTransition) && visitTransition.test(
                    newTransition)) {
