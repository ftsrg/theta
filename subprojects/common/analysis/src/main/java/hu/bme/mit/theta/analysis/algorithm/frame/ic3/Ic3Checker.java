...
public class Ic3Checker extends FrameBasedChecker<IC3Optimizations> {
    ...
    public Ic3Checker(
            ...
            IC3Optimizations optimizations,
            Logger logger) {
    ...
    public SafetyResult<PredState, Trace<ExplState, ExprAction>> check(UnitPrec prec) {
        ...
        while (true) {
            final ProofObligation counterExample =
                    checkCurrentFrameForInterSections(Not(monolithicExpr.getPropExpr()));
        ...
        while (!proofObligationsQueue.isEmpty()) {
            ...
            if (solverStatus.isSat()) {
                ...
                proofObligationsQueue.add(
                        new ProofObligation(
                                Cube.of(reachableExprInFormerFrame),
                                proofObligation.getTime() - 1));
            } else {
