...
sealed class Reason {
  ...
  open fun toExprs(): List<Expr<BoolType>> =
    reasons.map { it.toExprs() }.flatten().filter { it !is TrueExpr }
...
sealed class DerivedReason<E : Event>(
  ...
  override fun toExprs(): List<Expr<BoolType>> =
    listOf(rf.declRef, w.guardExpr) + wRfRelation.toExprs()
}
