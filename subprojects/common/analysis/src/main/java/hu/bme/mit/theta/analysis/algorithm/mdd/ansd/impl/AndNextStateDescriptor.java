 ...
 * constrains the target only, a full relation constrains source and target, and a don't-care prefix
 * is supplied by {@link LiftNextStateDescriptor}. Enumeration drives the exhaustive (no-default)
 * side and probes the other by key, so pass the structural/explicit operand as {@code lhs}.
 */
public final class AndNextStateDescriptor implements AbstractNextStateDescriptor {
    ...
    // correct whether an operand is identity or a Lift prefix leaves the other constraining.
