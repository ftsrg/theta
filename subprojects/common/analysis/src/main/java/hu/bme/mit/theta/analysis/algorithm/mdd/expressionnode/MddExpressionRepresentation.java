...
public class MddExpressionRepresentation implements RecursiveIntObjMapView<MddNode> {
    ...
    public RecursiveIntObjCursor<? extends MddNode> cursor(RecursiveIntObjMapView<?> constraint) {
        ...
        if (mddVariable.getLower().isPresent()) {
            ...
            while (true) {
                ...
                // This is needed because the constraint node might contain level-skips: of the
                ...
                if (!decl.getType().equals(BoolType.getInstance())
                        && !(decl.getType() instanceof EnumType)) { // TODO delete
    ...
    private static class Traverser implements Closeable {
        ...
        public static Traverser createConstrained(
                MddExpressionRepresentation rootRepresentation,
                Expr<BoolType> constraint,
                SolverPool solverPool) {
            ...
            while (true) {
                ...
                if (representation.explicitRepresentation.getCacheView().defaultValue() != null) {
                    ...
                    final LitExpr<?> literalToCache =
                            determineLiteralToCache(representation, valuation);

                    if (representation
                            .explicitRepresentation
                            .getCacheView()
                            .containsKey(LitExprConverter.toInt(literalToCache))) {

                        childNode =
                                representation
                                        .explicitRepresentation
                                        .getCacheView()
                                        .get(LitExprConverter.toInt(literalToCache));
                        ...
                        final Expr<BoolType> substitutedExpr =
                                ExprUtils.simplify(
                                        ...
                                        ImmutableValuation.builder()
                                                .put(representation.getDecl(), literalToCache)
                                                .build());
                        if (lower.isPresent()) {
                            final MddExpressionTemplate template =
                        ...
                        representation.explicitRepresentation.cacheNode(
                                LitExprConverter.toInt(literalToCache), childNode);
        ...
        private static LitExpr<?> determineLiteralToCache(
                MddExpressionRepresentation representation, Valuation valuation) {
            ...
            if (literal.isPresent()) {
                ...
                return LitExprConverter.toLitExpr(
                        generateMissingLiteral(representation), decl.getType());
            }
        }
        ...
        private static int generateMissingLiteral(MddExpressionRepresentation representation) {
            ...
            if (representation.mddVariable.isBounded()) {
                final IntSetView domain =
                        IntSetView.range(0, representation.mddVariable.getDomainSize());
                final IntSetView remaining =
                        domain.minus(representation.explicitRepresentation.getCacheView().keySet());
                if (remaining.isEmpty()) {
                    representation.explicitRepresentation.setComplete();
                ...
                final IntSetView cachedKeys =
                        representation.explicitRepresentation.getCacheView().keySet();
