...
fun orderVars(vars: List<VarDecl<*>>, events: Set<Stmt>): List<VarDecl<*>> {
    ...
    val affectedVars = events.associateWith { event ->
        StmtUtils.getVars(event)
    }

    val affectingEvents = vars.associateWith { varDecl ->
        events.filter { varDecl in affectedVars[it]!! }.toSet()
    }
    ...
    while (true) {
        val cogs = events.associateWith {
            affectedVars[it]!!.map { varDecl -> currentVarOrdering.indexOf(varDecl) }.fold(0, Int::plus)
                .toDouble() / affectedVars[it]!!.size.toDouble()
        }
        val newLocations = vars.associateWith { varDecl ->
            affectingEvents[varDecl]!!.map { cogs[it]!! }.fold(0.0, Double::plus) / affectingEvents[varDecl]!!.size.toDouble()
        }
...
private fun eventSpans(vars: List<VarDecl<*>>, events: Set<Stmt>) = events.map { event ->
