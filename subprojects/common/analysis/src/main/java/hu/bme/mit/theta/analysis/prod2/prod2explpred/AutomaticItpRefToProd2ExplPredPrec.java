...
public final class AutomaticItpRefToProd2ExplPredPrec
    ...
    private AutomaticItpRefToProd2ExplPredPrec(
            final AutoExpl autoExpl, final ExprSplitter exprSplitter) {
    ...
    public Prod2Prec<ExplPrec, PredPrec> toPrec(ItpRefutation refutation, int index) {
        ...
        final var explSelectedVars =
                ExprUtils.getVars(refExpr).stream()
                        .filter(autoExpl::isExpl)
                        .collect(Collectors.toSet());
        final var predSelectedExprs =
                exprSplitter.apply(refExpr).stream()
                        .filter(
                                expr ->
                                        !ExprUtils.getVars(expr).stream()
                                                .allMatch(autoExpl::isExpl))
                        .collect(Collectors.toSet());
    ...
    public Prod2Prec<ExplPrec, PredPrec> join(
        ...
        final var filteredPreds =
                joinedPred.getPreds().stream()
