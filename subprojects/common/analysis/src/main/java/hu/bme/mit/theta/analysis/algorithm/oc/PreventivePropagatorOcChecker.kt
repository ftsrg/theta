...
class PreventivePropagatorOcChecker<E : Event> : UserPropagatorOcChecker<E>() {
  ...
  private val Expr<BoolType>.prevented: Boolean
    get() = preventedClauses.any { this in it.value }
  ...
  override fun check(
    ...
    reads =
      events.keys.associateWith { v ->
        events[v]!!.values.flatten().filter { it.type == EventType.READ }
      }
    ...
    writes.forEach { (v, ws) ->
      ...
      for (w in ws) for (r in rs) {
        ...
        rels[w.clkId][r.clkId]?.let { wrReason ->
          for (w0 in ws) rels[w0.clkId][w.clkId]?.let { w0wReason -> // w0 -> w -> r
            rfs[v]
              ?.find { it.from == w0 && it.to == r }
              ?.let { rf -> // rf{w0->r}
