...
object MddExplicitRepresentationExtractor {
    ...
    fun transform(
        node: MddHandle, variable: MddVariableHandle
    ): MddHandle = transform(node, variable, UnaryOperationCache())

    fun transform(
        node: MddHandle, variable: MddVariableHandle, cache: UnaryOperationCache<MddHandle, MddHandle>
    ): MddHandle {
        ...
        if (node.isTerminal) {
            ...
            while (cursor.moveNext()) {
                val s = transform(
                    variable.lower.get().getHandleFor(cursor.value()), variable.lower.orElse(null), cache
                )
