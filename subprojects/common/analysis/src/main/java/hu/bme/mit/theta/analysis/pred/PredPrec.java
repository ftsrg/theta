...
/** Represents an immutable, simple predicate precision that is a set of predicates. */
public final class PredPrec implements Prec {
    ...
    public PredPrec join(final PredPrec other) {
        checkNotNull(other);
        final Collection<Expr<BoolType>> joinedPreds =
                ImmutableSet.<Expr<BoolType>>builder()
                        .addAll(this.predToNegMap.keySet())
                        .addAll(other.predToNegMap.keySet())
                        .build();
    ...
    public Collection<VarDecl<?>> getUsedVars() {
        return predToNegMap.keySet().stream()
                .map(ExprUtils::getVars)
                .reduce(
                        (vars1, vars2) ->
                                Streams.concat(vars1.stream(), vars2.stream())
