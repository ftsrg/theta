...
internal fun <LState : State, RState : State, DataState : State, LControl : State, RControl : State, LAction : Action, RAction : Action, LPrec : Prec, RPrec : Prec, DataPrec : Prec, LControlPrec : Prec, RControlPrec : Prec, MState : MultiState<LControl, RControl, DataState>, MControlState : MultiState<LControl, RControl, UnitState>, MAction : MultiAction<LAction, RAction>, MAnalysis : MultiAnalysis<LState, RState, DataState, LControl, RControl, LAction, RAction, LPrec, RPrec, DataPrec, LControlPrec, RControlPrec, MState, MControlState, MAction>, MLts : MultiLts<LState, RState, DataState, LControl, RControl, LAction, RAction, MState, MAction>> multiBuilderResult(
    analysis: MAnalysis, lts: MLts
): MultiBuilderResultPOJO<LState, RState, DataState, LControl, RControl, LAction, RAction, LPrec, RPrec, DataPrec, LControlPrec, RControlPrec, MState, MControlState, MAction, MLts> {
    val initFunc = MultiControlInitFunc(
        analysis.leftSide.controlInitFunc, analysis.rightSide.controlInitFunc, analysis::createControlInitState
    )
    ...
    val extractDataFromState: (MState) -> DataState = { mState ->
        mState.dataState
    }
    val extractControlPrec: (MultiPrec<LPrec, RPrec, DataPrec>) -> MultiPrec<LControlPrec, RControlPrec, UnitPrec> =
        { mPrec ->
            MultiPrec(
                analysis.leftSide.extractControlPrec.invoke(
                    mPrec.leftPrec()
                ), analysis.rightSide.extractControlPrec.invoke(mPrec.rightPrec()), UnitPrec.getInstance()
            )
        }
    val side = MultiAnalysisSide(
        analysis, initFunc, combineStates, extractControlState, extractDataFromState, extractControlPrec
    )
