...
(
    ...
    override fun getInitStates(prec: MultiPrec<LPrec, RPrec, DataPrec>?): MutableCollection<out MState> {
        prec!!
        ...
        val leftInitStates: Collection<LControl> = HashSet<LControl>(leftControlInitFunc.getInitStates(leftInitPrec))
        val rightInitStates: Collection<RControl> = HashSet<RControl>(rightControlInitFunc.getInitStates(rightInitPrec))
        val dataInitStates: Collection<DataState> = dataInitFunc.getInitStates(prec.dataPrec)
        return leftInitStates.flatMap { leftInitState ->
