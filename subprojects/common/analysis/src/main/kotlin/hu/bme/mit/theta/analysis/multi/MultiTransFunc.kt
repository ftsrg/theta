...
class MultiTransFunc<LState : State, RState : State, DataState : State, LControl : State, RControl : State,
...
(
    ...
    override fun getSuccStates(
        state: MState, action: MAction, prec: MultiPrec<LPrec, RPrec, DataPrec>?
        ...
        if (nextSide != MultiSide.RIGHT && action.leftAction != null) {
            leftTransFunc.getSuccStates(
                leftCombineStates(state.leftState, state.dataState), action.leftAction, prec!!.leftPrec
            ).map {
                createState(
                    leftExtractControlState(it!!), state.rightState, leftExtractDataState(it),
                    MultiSide.LEFT
                )
            }.forEach(succStates::add)
        }
        if (nextSide != MultiSide.LEFT && action.rightAction != null) {
            rightTransFunc.getSuccStates(
                rightCombineStates(state.rightState, state.dataState), action.rightAction, prec!!.rightPrec
            ).map {
                createState(
                    state.leftState, rightExtractControlState(it!!), rightExtractDataState(it),
                    MultiSide.RIGHT
                )
            }.forEach(succStates::add)
        }
