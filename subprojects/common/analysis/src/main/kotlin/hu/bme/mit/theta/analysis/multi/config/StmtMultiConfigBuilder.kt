...
>(
  private val product:
    MultiBuilderResultPOJO<
      ...
      ExprMultiState<LControl, RControl, UnitState>,
      StmtMultiAction<LAction, RAction>,
      StmtMultiLts<LState, RState, DataState, LControl, RControl, LAction, RAction>,
    >,
  ...
  >(
    ...
    enum class TraceCheckerType(
      val generator:
        (init: Expr<BoolType>, target: Expr<BoolType>, solver: ItpSolver) -> ExprTraceChecker<
            ItpRefutation
          >
    ) {
    ...
    override fun getTraceChecker(): ExprTraceChecker<ItpRefutation> =
      traceCheckerType.generator(BoolExprs.True(), prop, solverFactory.createItpSolver())
  }
  ...
  >(
    ...
    enum class TraceCheckerType(
      val generator:
        (init: Expr<BoolType>, target: Expr<BoolType>, solver: UCSolver) -> ExprTraceChecker<
            VarsRefutation
          >
    ) {
    ...
    override fun getTraceChecker(): ExprTraceChecker<VarsRefutation> =
      traceCheckerType.generator(BoolExprs.True(), prop, solverFactory.createUCSolver())
  }
}
