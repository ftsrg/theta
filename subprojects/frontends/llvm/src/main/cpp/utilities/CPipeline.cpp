...
bool CPipeline::executeClang() {
    ...
    if (returnCode == -1) {
        std::cerr << "ERROR: failed to execute clang: "
                     << (clangErrors.empty() ? "Unknown error." : clangErrors) << "\n";
        return false;
    }

    if (returnCode != 0) {
        std::cerr << "ERROR: clang exited with a non-zero exit code.\n";
        return false;
    }
    ...
    if (module == nullptr) {
        std::cout << "Could not parse module" << std::endl;
        return false;
    }
    return true;
}
