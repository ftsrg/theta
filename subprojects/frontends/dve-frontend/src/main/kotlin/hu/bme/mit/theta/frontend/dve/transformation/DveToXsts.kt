...
private class TransformContext(private val model: DveModel) {
    ...
    val localVars: List<VarDecl<*>> get() = emptyList()
    ...
    private fun declareDataVariables() {
        ...
        for (proc in model.processes)
            for (decl in proc.variables) declareVarOrArray(decl, proc.name)
    }
    ...
    private fun buildInit() {
        ...
        for (proc in model.processes)
            for (decl in proc.variables) initVarOrArray(decl, proc.name, fExprs)
            ...
            is DveVarOrArrayDecl.Array -> {
                ...
                for (i in 0 until decl.decl.size) {
                    ...
                    val init = decl.decl.initialValues?.getOrNull(i)
                        ?.let { translateExpr(it, null) } ?: iLit(0)
                    fExprs += Eq(v.ref, init)
                }
            }
        }
    ...
    private fun buildTrans() {
        ...
        for (proc in model.processes) {
            for (trans in proc.transitions) {
                when (val sync = trans.sync) {
                    ...
                    null ->
                        branches += buildLocalTransBranch(proc, trans, anyCommitted)
                }
            }
        }
        ...
        for (ch in model.channels) {
            if (ch.bufferSize == 0) {
                ...
                for ((sProc, sTrans) in sends)
                    for ((rProc, rTrans) in recvs) {
                        if (sProc.name == rProc.name) continue
                        branches += buildRendezvousBranch(sProc, sTrans, rProc, rTrans, anyCommitted)
                    }
            } else {
                for ((sProc, sTrans) in (sendMap[ch.name] ?: emptyList()))
                    branches += buildBufferedSendBranch(ch, sProc, sTrans, anyCommitted)
                for ((rProc, rTrans) in (recvMap[ch.name] ?: emptyList()))
                    branches += buildBufferedRecvBranch(ch, rProc, rTrans, anyCommitted)
            }
        }
    ...
    private fun buildAnyCommittedExpr(): Expr<BoolType>? {
        ...
        for (proc in model.processes) {
            val pc   = pcVars[proc.name]!!
            for (state in proc.committedStates)
                terms += Eq(pc.ref, stateLit(proc.name, state))
        }
    ...
    private fun buildRendezvousBranch(
        sProc: DveProcess, sTrans: DveTransition,
        rProc: DveProcess, rTrans: DveTransition,
        anyCommitted: Expr<BoolType>?,
    ): Stmt {
        ...
        var guard: Expr<BoolType> = SmartBoolExprs.And(
        ...
        val tmpVars = sSend.values.mapIndexed { i, valExpr ->
        ...
        for ((i, lval) in rRecv.variables.withIndex())
            if (i < tmpVars.size)
    ...
    private fun buildBufferedSendBranch(
        ch: DveChannelDecl, proc: DveProcess, trans: DveTransition,
        anyCommitted: Expr<BoolType>?,
    ): Stmt {
        ...
        for ((i, valExpr) in send.values.withIndex()) {
            ...
            for (slot in 0 until ch.bufferSize) {
                val sv = chanVars["${ch.name}.slot${slot}.field${i}"]
    ...
    private fun buildBufferedRecvBranch(
        ...
        for ((i, lval) in recv.variables.withIndex()) {
            val slot0Var = chanVars["${ch.name}.slot0.field$i"]
        ...
        if (ch.typedFields.isNotEmpty()) {
            for (slot in 0 until ch.bufferSize - 1)
                for (field in ch.typedFields.indices) {
                    val sv     = chanVars["${ch.name}.slot${slot}.field$field"]
                        ?: Decls.Var("${ch.name}_slot${slot}_field$field", IntType.getInstance())
                    val svNext = chanVars["${ch.name}.slot${slot + 1}.field$field"]
    ...
    private fun buildConditionalAssign(
    ...
    private fun buildLvalueAssignment(lval: DveLValue, rhs: Expr<*>, ctx: String): Stmt =
        when (lval) {
            is DveLValue.VarLValue -> {
                ...
                val v = lookupVar(procCtx, lval.varName)
                    ?: throw IllegalArgumentException("Unknown variable: ${lval.processName}.${lval.varName}")
    ...
    private fun buildArrayAssignment(
        procCtx: String, arrName: String, idxExpr: Expr<*>, rhs: Expr<*>,
    ): Stmt {
    ...
    fun translateExpr(expr: DveExpression, contextProcess: String?): Expr<*> = when (expr) {
        ...
        is DveExpression.UnaryExpr -> when (expr.op) {
            DveUnaryOp.NEG    -> Neg(translateExpr(expr.operand, contextProcess))
            DveUnaryOp.NOT    -> throw UnsupportedOperationException(
                "Unary NOT in pure integer context is not supported.")
            DveUnaryOp.BITNOT -> throw UnsupportedOperationException("Bitwise NOT (~) is not supported.")
        }
        is DveExpression.BinaryExpr -> translateBinaryIntExpr(expr, contextProcess)
    }

    private fun translateBinaryIntExpr(
        expr: DveExpression.BinaryExpr, contextProcess: String?,
    ): Expr<*> = when (expr.op) {
        DveBinaryOp.ADD -> Add(translateExpr(expr.left, contextProcess), translateExpr(expr.right, contextProcess))
        DveBinaryOp.SUB -> Sub(translateExpr(expr.left, contextProcess), translateExpr(expr.right, contextProcess))
    ...
    fun dveExprToBool(expr: DveExpression, contextProcess: String?): Expr<BoolType> = when (expr) {
        is DveExpression.LiteralExpr  ->
            if (expr.value != 0) BoolExprs.True() else BoolExprs.False()
        is DveExpression.VarRefExpr   ->
            Neq(translateExpr(expr, contextProcess), iLit(0))
        is DveExpression.ArrayAccessExpr ->
            Neq(translateExpr(expr, contextProcess), iLit(0))
        is DveExpression.ProcessStateExpr -> {
            val pc    = pcVars[expr.processName]
                ?: throw IllegalArgumentException("Unknown process: ${expr.processName}")
            if (expr.stateName !in (model.processes.find { it.name == expr.processName }?.states ?: emptyList()))
    ...
    private fun translateBinaryBoolExpr(
        expr: DveExpression.BinaryExpr, contextProcess: String?,
    ): Expr<BoolType> = when (expr.op) {
        DveBinaryOp.AND -> SmartBoolExprs.And(
            dveExprToBool(expr.left, contextProcess),
            dveExprToBool(expr.right, contextProcess))
        DveBinaryOp.OR  -> SmartBoolExprs.Or(
            dveExprToBool(expr.left, contextProcess),
            dveExprToBool(expr.right, contextProcess))
        DveBinaryOp.EQ  -> Eq (translateExpr(expr.left, contextProcess), translateExpr(expr.right, contextProcess))
        DveBinaryOp.NEQ -> Neq(translateExpr(expr.left, contextProcess), translateExpr(expr.right, contextProcess))
        DveBinaryOp.LT  -> Lt (translateExpr(expr.left, contextProcess), translateExpr(expr.right, contextProcess))
        DveBinaryOp.LEQ -> Leq(translateExpr(expr.left, contextProcess), translateExpr(expr.right, contextProcess))
        DveBinaryOp.GT  -> Gt (translateExpr(expr.left, contextProcess), translateExpr(expr.right, contextProcess))
        DveBinaryOp.GEQ -> Geq(translateExpr(expr.left, contextProcess), translateExpr(expr.right, contextProcess))
        DveBinaryOp.ADD, DveBinaryOp.SUB, DveBinaryOp.MUL,
        DveBinaryOp.DIV, DveBinaryOp.MOD ->
            Neq(translateBinaryIntExpr(expr, contextProcess), iLit(0))
        DveBinaryOp.BITAND, DveBinaryOp.BITOR, DveBinaryOp.BITXOR,
        DveBinaryOp.SHL,    DveBinaryOp.SHR ->
    ...
    companion object {
        ...
        fun iLit(value: Int): hu.bme.mit.theta.core.type.inttype.IntLitExpr =
            IntExprs.Int(value)
    }
}
