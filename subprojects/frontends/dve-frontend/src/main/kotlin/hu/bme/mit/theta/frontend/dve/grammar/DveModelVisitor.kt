 ...
 *
 * Qualified identifiers of the form `ProcessName.identifier` are syntactically ambiguous:
 * `identifier` may be either a variable name or a state name in that process. Resolution is done in
...
class DveModelVisitor : dveBaseVisitor<Any>() {
  ...
  override fun visitModel(ctx: dveParser.ModelContext): DveModel {
    ...
    for (decl in ctx.topDecl()) {
      ...
      when {
        decl.varOrArrayDecl() != null ->
          globalVars += buildVarOrArrayDecls(decl.varOrArrayDecl(), isConst)
  ...
  private fun buildVarOrArrayDecls(
    ctx: dveParser.VarOrArrayDeclContext,
    isConst: Boolean = false,
  ): List<DveVarOrArrayDecl> {
    val type = buildVarType(ctx.varType())
    return ctx.varOrArrayItem().map { item ->
      when (item) {
        is dveParser.ScalarItemDeclContext ->
          DveVarOrArrayDecl.Scalar(
            DveVariableDecl(
              name = item.ID().text,
              type = type,
              initialValue = item.expr()?.let { buildExpr(it) },
              isConst = isConst,
            )
          )
        is dveParser.ArrayItemDeclContext ->
        ...
        else -> error("Unknown varOrArrayItem: $item")
      }
    }
  }
  ...
  private fun buildProcess(ctx: dveParser.ProcessDeclContext): DveProcess {
    ...
    val localVars = buildList {
      body.localDecl().forEach { ld ->
        ...
        addAll(buildVarOrArrayDecls(ld.varOrArrayDecl(), isConst))
      }
    }
  ...
  private fun buildSyncAction(ctx: dveParser.SyncActionContext): DveSyncAction =
    when (ctx) {
      is dveParser.SyncSendContext ->
        DveSyncAction.Send(channelName = ctx.ID().text, values = ctx.expr().map { buildExpr(it) })
  ...
  private fun buildLValue(ctx: dveParser.LvalueContext): DveLValue =
    when (ctx) {
      ...
      is dveParser.QualLValueContext ->
        DveLValue.VarLValue(processName = ctx.ID(0).text, varName = ctx.ID(1).text)
  ...
  private fun buildAtom(ctx: dveParser.AtomContext): DveExpression =
    when (ctx) {
      ...
      is dveParser.NotExprContext -> DveExpression.UnaryExpr(DveUnaryOp.NOT, buildExpr(ctx.expr()))
      is dveParser.NotPrefixExprContext ->
        DveExpression.UnaryExpr(DveUnaryOp.NOT, buildAtom(ctx.atom()))
      is dveParser.IntLitContext -> DveExpression.LiteralExpr(ctx.INT_LITERAL().text.toInt())
  ...
  /**
   * Resolves `processName.identifier` to either a [DveExpression.ProcessStateExpr] (if `identifier`
   * is a known state of `processName`) or a [DveExpression.VarRefExpr].
   */
