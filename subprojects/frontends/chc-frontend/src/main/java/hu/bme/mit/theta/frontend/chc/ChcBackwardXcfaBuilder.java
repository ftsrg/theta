...
public class ChcBackwardXcfaBuilder extends CHCBaseVisitor<Object> implements ChcXcfaBuilder {
    ...
    public Object visitChc_assert(CHCParser.Chc_assertContext ctx) {
        ...
        if (ctx.chc_tail() != null) {
            ...
            createCalls(procedure, middle, procedure.getFinalLoc(), ctx.chc_tail().u_pred_atom(), vars);
        } else {
            String procName;
            ...
            XcfaEdge edge = XcfaEdge.of(procedure.getInitLoc(), procedure.getFinalLoc(), List.of(XcfaLabel.Stmt(returnTrue)));
            procedure.addEdge(edge);
        }
    ...
    public Object visitChc_query(CHCParser.Chc_queryContext ctx) {
        ...
        XcfaEdge edge = XcfaEdge.of(mainProcedure.getInitLoc(), middle, getTailConditionLabels(ctx.chc_tail(), vars));
        mainProcedure.addEdge(edge);
        createCalls(mainProcedure, middle, mainProcedure.getErrorLoc(), ctx.chc_tail().u_pred_atom(), vars);
    ...
    private VarDecl<?> getOutParam(XcfaProcedure.Builder procedure) {
        Optional<Map.Entry<VarDecl<?>, XcfaProcedure.Direction>> param = procedure.getParams().entrySet()
                        .stream().filter(entry -> entry.getValue() == XcfaProcedure.Direction.OUT).findAny();
        return param.map(Map.Entry::getKey).orElse(null);
    }

    private void createCalls(XcfaProcedure.Builder builder, XcfaLocation start, XcfaLocation end, List<CHCParser.U_pred_atomContext> uPreds, Map<String, VarDecl<?>> localVars) {
        ...
        for (CHCParser.U_pred_atomContext uPred : uPreds) {
            ...
            XcfaEdge callEdge = XcfaEdge.of(from, middle, List.of(XcfaLabel.ProcedureCall((List<Expr<?>>) params, calledProcedure.getName())));
            builder.addEdge(callEdge);

            XcfaEdge assertEdge = XcfaEdge.of(middle, to, List.of(XcfaLabel.Stmt(AssumeStmt.of(ret.getRef()))));
