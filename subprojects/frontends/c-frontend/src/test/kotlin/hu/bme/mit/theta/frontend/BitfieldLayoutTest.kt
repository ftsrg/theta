...
class BitfieldLayoutTest {
  ...
  fun packedBitfieldsShareAUnit() {
    ...
    val layout =
      BitfieldLayout.compute(listOf(ordinary(8), bitfield(8, 2), bitfield(8, 2), bitfield(8, 4)))
  ...
  fun aBitfieldThatOverflowsTheUnitStartsANewOne() {
    ...
    val layout = BitfieldLayout.compute(listOf(bitfield(8, 6), bitfield(8, 6), bitfield(8, 6)))
    assertEquals(3, layout.unitCount)
    assertEquals(listOf(0, 1, 2), layout.slots.map { it.unitIndex })
  ...
  fun anOrdinaryMemberBreaksABitfieldRun() {
    ...
    val layout = BitfieldLayout.compute(listOf(bitfield(8, 4), ordinary(32), bitfield(8, 4)))
    assertEquals(3, layout.unitCount)
    assertEquals(listOf(0, 1, 2), layout.slots.map { it.unitIndex })
  }
  ...
  fun namedZeroWidthBitfieldIsABoundaryWithNoStorage() {
    ...
    val layout = BitfieldLayout.compute(listOf(bitfield(8, 4), bitfield(8, 0), bitfield(8, 4)))
