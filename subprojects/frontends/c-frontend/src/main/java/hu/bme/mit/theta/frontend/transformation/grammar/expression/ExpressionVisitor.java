...
public class ExpressionVisitor extends CBaseVisitor<Expr<?>> {
    ...
    private void addPreStatements(CStatement ifTrue) {
        if (ifTrue instanceof CCompound) {
            if (ifTrue.getPreStatements() != null) {
                preStatements.add(ifTrue.getPreStatements());
            }
            if (ifTrue.getPostStatements() != null) {
                postStatements.add(ifTrue.getPostStatements());
            }
    ...
    public Expr<?> visitPrimaryExpressionConstant(CParser.PrimaryExpressionConstantContext ctx) {
        ...
        if (text.contains(".") || text.contains("e")) {
            ...
            if (isLongLong && isUnsigned) {
                type = CComplexType.getUnsignedLongLong();
            } else if (isLongLong) {
                type = CComplexType.getSignedLongLong();
            } else if (isLong && isUnsigned) {
                type = CComplexType.getUnsignedLong();
            } else if (isLong) {
                type = CComplexType.getSignedLong();
            } else if (isUnsigned) {
                type = CComplexType.getUnsignedInt();
            } else {
                type = CComplexType.getSignedInt();
            }
