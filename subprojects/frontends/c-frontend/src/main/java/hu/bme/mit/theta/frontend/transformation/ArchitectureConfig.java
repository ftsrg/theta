...
public class ArchitectureConfig {
    ...
    public enum MemoryModelType {
        ...
        /**
         * Whether a pointer is a single flat scalar address (base + offset folded together) rather
         * than the 2-D {@code (base, offset)} pair. Both {@link #flat} and {@link #bytes} use flat
         * addressing; {@link #bytes} additionally makes every cell one byte.
         */
        public boolean flatAddressing() {
            return this == flat || this == bytes;
        }

        /**
         * Whether every memory cell is one byte ({@code Bv8}), with a wider scalar read as a {@code
         * Concat} of its bytes and written as an {@code Extract}-and-split of them.
         */
        public boolean byteAddressed() {
            return this == bytes;
        }
    }
