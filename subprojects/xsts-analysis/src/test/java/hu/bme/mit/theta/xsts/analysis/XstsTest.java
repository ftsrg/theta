...
//				{ "src/test/resources/model/crossroad_composite.xsts", "src/test/resources/property/both_green.prop", true, XstsConfigBuilder.Domain.EXPL}
	...
	public void test() throws IOException {
		...
		try {
			...
			final XstsConfig<?, ?, ?> configuration = new XstsConfigBuilder(domain, XstsConfigBuilder.Refinement.FW_BIN_ITP, Z3SolverFactory.getInstance()).predSplit(XstsConfigBuilder.PredSplit.WHOLE).initPrec(XstsConfigBuilder.InitPrec.PROP).pruneStrategy(PruneStrategy.FULL).maxEnum(250).logger(logger).build(xsts);
