...
class PartialXcfaManager extends InstantiatableSymbol<XCFA> implements Scope {
	...
	public PartialXcfaManager() {
		symbolTable = new SymbolTable();
		vars = new ArrayList<>();
		processes = new ArrayList<>();
	}

	void addContext(final XcfaDslParser.SpecContext context) {
		...
		context.processDecls.forEach(processDeclContext -> {
			var optProc = symbolTable.get(processDeclContext.ID().toString());
			if (optProc.isPresent()) {
				((XcfaProcessSymbol)optProc.get()).addContext(processDeclContext);
			} else {
