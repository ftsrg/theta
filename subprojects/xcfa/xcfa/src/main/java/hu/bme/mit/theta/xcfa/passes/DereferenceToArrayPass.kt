...
class DereferenceToArrayPass : ProcedurePass {
   ...
   *    )
   */
  private fun Dereference<*, *, *>.getArrayWrite(expr: Expr<*>, xcfa: XcfaBuilder): AssignStmt<*> {
    val arrayType = ArrayType.of(this.array.type, ArrayType.of(this.offset.type, this.type))
    val arrays = this.getArrays(xcfa)
    return AssignStmt.of(
      cast(arrays, arrayType),
      cast(
        ArrayWriteExpr.of(
          cast(arrays.ref, arrayType),
          cast(this.array.getArrayReads(xcfa), arrayType.indexType),
          ArrayWriteExpr.of(
            cast(
              ArrayReadExpr.of(
                cast(arrays.ref, arrayType),
            ...
            cast(this.offset.getArrayReads(xcfa), arrayType.elemType.indexType),
            cast(expr.getArrayReads(xcfa), arrayType.elemType.elemType),
          ),
        ),
        arrayType,
      ),
