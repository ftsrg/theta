...
class LbePass : ProcedurePass {
    ...
    private fun removeMiddleLocation(location: XcfaLocation): Boolean {
        ...
        if (location.outgoingEdges.stream().anyMatch { edge -> edge.getFlatLabels().any {it is InvokeLabel} } || (level == LBELevel.LBE_LOCAL && !atomicPhase && location.outgoingEdges.stream().anyMatch { edge: XcfaEdge -> isNotLocal(edge) })) {
            return false
        }
