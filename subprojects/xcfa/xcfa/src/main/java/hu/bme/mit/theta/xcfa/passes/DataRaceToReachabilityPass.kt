...
/**
 * Reduces data race checking to reachability checking by adding write access flags for each
 * global variable write access, and checks for multiple access and each global variable access
 * (writes and reads).
 */
class DataRaceToReachabilityPass : ProcedurePass {
  ...
  private val VarDecl<*>.writeFlag: VarDecl<IntType> get() = writeFlagVars[this]!!
  private val VarDecl<*>.readFlag: VarDecl<IntType> get() = readFlagVars[this]!!
  ...
  override fun run(builder: XcfaProcedureBuilder): XcfaProcedureBuilder {
    ...
    while (locationsToVisit.isNotEmpty()) {
      ...
      loc.outgoingEdges.toSet().forEachIndexed { index0, edge ->
        ...
        val newLabels =
          edge.getFlatLabels().mapIndexed { index1, label ->
            ...
            if (vars.isEmpty()) return@mapIndexed listOf(label) to null
            anyChange = true
            ...
            if (branching && firstLabel) {
              ...
              vars.forEach { (v, access) ->
                ...
                unsetLabels.add(StmtLabel(AssignStmt.of(v.readFlag, Int(0))))
              }
              return@mapIndexed listOf(
                label,
                SequenceLabel(unsetLabels)
              ) to null
            }
              ...
              vars
                .flatMap { (v, access) ->
                  listOf(Eq(v.writeFlag.ref, Int(0))) +
                    if (access.isWritten) listOf(Eq(v.readFlag.ref, Int(0)))
                    else listOf()
                }
                .let { if (it.size == 1) it.first() else And(it) }
            val setLabels = mutableListOf<XcfaLabel>()
            val unsetLabels = mutableListOf<XcfaLabel>()
            vars.forEach { (v, access) ->
              if (access.isWritten) {
                setLabels.add(StmtLabel(AssignStmt.of(v.writeFlag, Int(1))))
                unsetLabels.add(StmtLabel(AssignStmt.of(v.writeFlag, Int(0))))
              }
              if (access.isRead) {
                setLabels.add(StmtLabel(AssignStmt.of(v.readFlag, Int(1))))
                unsetLabels.add(StmtLabel(AssignStmt.of(v.readFlag, Int(0))))
              }
            }
            listOf(
              SequenceLabel(listOf(StmtLabel(AssumeStmt.of(assertion))) + setLabels),
              label,
              SequenceLabel(unsetLabels),
            ) to StmtLabel(AssumeStmt.of(Not(assertion)))
          }
        ...
        if (anyChange) {
          ...
          newLabels.forEachIndexed { index1, (labels, errorLabel) ->
            ...
            labels.filter { !(it is SequenceLabel && it.labels.isEmpty()) }.forEachIndexed { index2, label ->
              val target =
                if (index1 == newLabels.size - 1 && index2 == labels.size - 1) edge.target
                else
                  XcfaLocation(
                    "${edge.source.name}_dr_${index0}_${index1}_${index2}",
      ...
      if (branching && allVarsToCheck.isNotEmpty()) {
        val newLoc = XcfaLocation("${loc.name}_dr", metadata = loc.metadata).also { builder.addLoc(it) }
        builder.addLoc(newLoc)
        val setLabels = mutableListOf<XcfaLabel>()
        val assertion = allVarsToCheck
          .map { v ->
            setLabels.add(StmtLabel(AssignStmt.of(v.readFlag, Int(1))))
            Eq(v.writeFlag.ref, Int(0))
          }
          .let { if (it.size == 1) it.first() else And(it) }
        ...
        val assumeLabel = StmtLabel(AssumeStmt.of(assertion), choiceType = ChoiceType.MAIN_PATH)
        val positiveLabel = SequenceLabel(listOf(assumeLabel) + setLabels)
        val assumeEdge = XcfaEdge(loc, newLoc, positiveLabel, metadata = EmptyMetaData)
        builder.addEdge(assumeEdge)
        builder.createErrorLoc()
        val errorLabel = StmtLabel(AssumeStmt.of(Not(assertion)), choiceType = ChoiceType.ALTERNATIVE_PATH)
        val errorEdge = XcfaEdge(loc, builder.errorLoc.get(), errorLabel, metadata = EmptyMetaData)
        builder.addEdge(errorEdge)
      }
    }
  ...
  private fun collectPotentialRacingVars(builder: XcfaProcedureBuilder): Set<VarDecl<*>> {
    if (potentialRacingVars == null) {
          ...
          .filter { v ->
            val usingThreads =
              builder.parent.getProcedures().count { b ->
                ...
                edges.any { e ->
                  e.getFlatLabels().any { l -> l.collectVarsWithAccessType().containsKey(v) }
                }
              }
            usingThreads > 1
          }
      ...
      val initializeFlags = potentialRacingVars!!.flatMap { v ->
      ...
      val newLoc = XcfaLocation("${initProcedure.initLoc.name}_dr", metadata = initProcedure.initLoc.metadata)
      ...
      val initEdge = XcfaEdge(initProcedure.initLoc, newLoc, SequenceLabel(initializeFlags), metadata = EmptyMetaData)
