...
| | `PthreadArrayHandleUnrollPass` | unroll `for (i…) pthread_create(&t[i], …)` so the handle index is a constant | **before `CLibraryFunctionsPass`**, which rejects a non-constant handle index, and **before `ReferenceElimination`**, which folds `&t[i]` away. There is no `SimplifyExprsPass` this early, which is why it substitutes the loop variable itself. Letting simplification do it instead has no valid pass order: simplification must follow `ReferenceElimination` (or it folds an object's naming variable into its base id and later passes stop recognising the object), `ReferenceElimination` must follow the three passes that read `&x` as a reference, and `CLibraryFunctionsPass` cannot follow it at all because an address-taken local is re-based onto a runtime counter, leaving a thread handle with no static identity |
