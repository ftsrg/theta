...
public final class XcfaProcedure {
    ...
    public String toDot(Collection<String> cexLocations, Collection<XcfaEdge> cexEdges) {
        ...
        for (XcfaLocation location : getLocs()) {
            ...
            if (cexLocations.contains(location.getName())) {
                ret.append(("[color=red]"));
            } else if (location.isErrorLoc()) {
                ret.append("[xlabel=err]");
            } else if (location.isEndLoc()) {
                ret.append("[xlabel=final]");
            } else if (getInitLoc() == location) {
                ret.append("[xlabel=start]");
            }
            ret.append(";\n");
        }
