package hu.bme.mit.theta.xcfa.transformation.utils;
...
import static hu.bme.mit.theta.xcfa.transformation.ArchitectureConfig.architecture;
...
public class CIntTypeUtils {
	...
	public static NamedType getcTypeMetadata(Object o) {
		...
		if(cTypeOptional.isPresent() && cTypeOptional.get() instanceof CSimpleType) {
			CSimpleType cSimpleType = (CSimpleType) cTypeOptional.get();
			checkState(cSimpleType instanceof NamedType);
			return (NamedType) cSimpleType;
		} else return null;
	}
	...
	static private NamedType deduceTypeBinary(NamedType typeA, NamedType typeB) {
		NamedType resultType = CSimpleTypeFactory.NamedType("int");
	...
	static private NamedType promoteToInteger(NamedType type) {
		...
		if(type.getNamedType().contains("char")) {
			typeCopy = CSimpleTypeFactory.NamedType("int");
