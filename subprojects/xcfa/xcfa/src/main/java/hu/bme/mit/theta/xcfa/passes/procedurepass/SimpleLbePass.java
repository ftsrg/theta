...
public class SimpleLbePass extends ProcedurePass {
    ...
    public enum LBELevel {
        ...
        /**
         * Enables collapsing of sequential edges of a location where the number of incoming edges
         * to the location is exactly 1. A new edge is created for every outgoing edge of the
         * location combined with the labels of the incoming edge. Parallel edges are not
         * collapsed.
         */
        LBE_SEQ,
    ...
    /**
     * Removes locations whose incoming degree is 1. A new edge is created for every outgoing edge
     * of the location combined with the labels of the incoming edge as a sequence (the labels of
     * the incoming edge will be the first in the sequence).
     */
     ...
     *
     * @param location         the location from where the parallel edges start that we want to
     *                         remove
     * @param locationsToVisit Adds the targets of parallel edges to this list (new parallel edges
     *                         and snakes can appear in these locations)
     */
    ...
    /**
     * Collapses the incoming and outgoing edges of a location whose incoming and outgoing degree is
     * 1.
     *
     ...
     * @param removedLocations The list of removed locations: the collapsed location is added to
     *                         this list
     */
    ...
    /**
     * Wraps edge labels to a {@link hu.bme.mit.theta.xcfa.model.XcfaLabel.SequenceLabel} if the
     * edge does not have exactly one label. If the labels contain one
     * {@link hu.bme.mit.theta.xcfa.model.XcfaLabel.NondetLabel}, the NondetLabel's labels are
     * returned to simplify the formula.
     *
    ...
    /**
     * Removes a location whose incoming degree is 1. A new edge is created for every outgoing edge
     * of the location combined with the labels of the incoming edge as a sequence (the labels of
     * the incoming edge will be the first in the sequence).
     *
