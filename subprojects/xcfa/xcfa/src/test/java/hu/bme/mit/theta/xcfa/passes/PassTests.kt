...
class PassTests {
    ...
    fun testPass(input: XcfaProcedureBuilder, output: XcfaProcedureBuilder, passes: List<ProcedurePass>) {
        val actualOutput = passes.fold(input) {acc, procedurePass -> procedurePass.run(acc) }.build(dummyXcfa)
