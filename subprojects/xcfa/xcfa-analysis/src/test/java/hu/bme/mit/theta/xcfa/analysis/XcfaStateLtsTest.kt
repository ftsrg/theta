...
class XcfaStateLtsTest {
  ...
  fun testApply() {
      ...
      xcfa("example") {
        val proc1 =
          procedure("proc1") {
            ...
            edges.add((init to final) { b assign a.ref })
          }
        val main =
          procedure("main") {
            val tmp = "tmp" type IntExprs.Int()
            edges.add((init to "L1") { proc1("1", tmp.ref) })
            edges.add(("L1" to "L2") { tmp.start(proc1, tmp.ref) })
            edges.add(("L2" to final) { tmp.join() })
          }
    ...
    expectations.add {
      it.processes[0]!!.locs.size == 2 &&
        it.processes[0]!!.locs.peek() == edges[0].source &&
    ...
    expectations.add {
      it.processes[0]!!.locs.size == 2 &&
        it.processes[0]!!.locs.peek() == edges[0].target &&
        lts.getEnabledActionsFor(it).size == 1 &&
        sporLts.getEnabledActionsFor(it).size == 1 &&
        aasporLts.getEnabledActionsFor(it, emptyList(), ExplPrec.empty()).size == 1
    }
    ...
    expectations.add {
      it.processes[0]!!.locs.size == 1 &&
        it.processes[0]!!.locs.peek() == edges[1].target &&
        lts.getEnabledActionsFor(it).size == 1 &&
        sporLts.getEnabledActionsFor(it).size == 1 &&
        aasporLts.getEnabledActionsFor(it, emptyList(), ExplPrec.empty()).size == 1
    }
    ...
    expectations.add {
      it.processes[0]!!.locs.size == 1 &&
        it.processes[0]!!.locs.peek() == edges[3].target &&
        lts.getEnabledActionsFor(it).size == 1 &&
        sporLts.getEnabledActionsFor(it).size == 1 &&
        aasporLts.getEnabledActionsFor(it, emptyList(), ExplPrec.empty()).size == 1
    }
...
private fun XcfaState<*>.foreignKey(): Int? = processes.keys.firstOrNull { key -> key != 0 }
