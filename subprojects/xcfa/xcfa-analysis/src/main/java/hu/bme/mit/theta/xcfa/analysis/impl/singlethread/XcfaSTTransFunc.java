...
public class XcfaSTTransFunc<S extends ExprState, A extends StmtAction, P extends Prec> implements
    TransFunc<XcfaState<S>, A, XcfaPrec<P>> {
    ...
    public static <S extends ExprState, A extends StmtAction, P extends Prec> XcfaSTTransFunc<S, A, P> create(
    ...
    private void handleFunctionPushPop(XcfaSTStateStack<S> state, XcfaSTAction action,
        XcfaSTStateStack<S> newState) {
        if (action.getLabels().size() > 0 && action.getLabels()
            .get(0) instanceof XcfaLabel.ProcedureCallXcfaLabel) {
            XcfaLabel.ProcedureCallXcfaLabel callLabel = (XcfaLabel.ProcedureCallXcfaLabel) action.getLabels()
                .get(0);
            Optional<XcfaProcedure> calledProcedure = state.getCurrentLoc().getParent().getParent()
                .getProcedures()
                .stream().filter(procedure -> callLabel.getProcedure().equals(procedure.getName()))
                .findAny();
            if (calledProcedure.isPresent()) {
                Map<VarDecl<?>, VarDecl<?>> reverseVarLut = state.getReverseVars();
                XcfaLabel.ProcedureCallXcfaLabel originalCallLabel = (XcfaLabel.ProcedureCallXcfaLabel) callLabel.accept(
                    new XcfaLabelVarReplacer(), reverseVarLut);
                newState.push(calledProcedure.get().getParamInitLoc(originalCallLabel));
            }
        }
        if (newState.getCurrentLoc().isEndLoc()
            && newState.getCurrentLoc().getParent() != newState.getCurrentLoc().getParent()
            .getParent().getMainProcedure()) {
            newState.pop();
        }
    }

    @Override
    public Collection<? extends XcfaState<S>> getSuccStates(final XcfaState<S> inState,
        final A inAction, final XcfaPrec<P> prec) {
        ...
        for (final S succState : transFunc.getSuccStates(state.getGlobalState(), inAction,
            globalPrec)) {
            final XcfaSTState<S> newState = state.withState(succState)
                .withLocation(action.getTarget());
            if (FunctionInlining.inlining != FunctionInlining.InlineFunctions.ON) {
                handleFunctionPushPop((XcfaSTStateStack<S>) state, action,
                    (XcfaSTStateStack<S>) newState);
            }
