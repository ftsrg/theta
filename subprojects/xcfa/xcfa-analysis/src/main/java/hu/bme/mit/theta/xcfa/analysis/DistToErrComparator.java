...
public class DistToErrComparator implements ArgNodeComparator {
	private static final long serialVersionUID = -6915823336852930451L;
	...
	public DistToErrComparator(final XCFA xcfa, final XcfaLocation errLoc) {
		this(xcfa, errLoc, 1, 0);
	}

	public DistToErrComparator(final XCFA xcfa, final XcfaLocation errLoc, final int errorWeight, final int depthWeight) {
		this.xcfa = xcfa;
		this.errLoc = errLoc;
	...
	private int getWeightedDistance(final ArgNode<? extends State, ? extends Action> node) {
		checkArgument(node.getState() instanceof XcfaState, "CfaState expected.");
		final XcfaState<?, ?, ?> state = (XcfaState<?, ?, ?>) node.getState();
		final int distanceToError = getDistanceToError(state.getLocations());
		if (distanceToError == MAX_VALUE) {
	...
	private int getDistanceToError(final XcfaLocation loc) {
		...
		return distancesToError.getOrDefault(loc, MAX_VALUE);
	}

	private int getDistanceToError(final Map<Integer, XcfaLocation> locs) {
		return locs.values().stream().mapToInt(this::getDistanceToError).min().orElse(MAX_VALUE);
	}

	static Map<XcfaLocation, Integer> calculateDistancesToError(final XCFA xcfa, final XcfaLocation errLoc) {
