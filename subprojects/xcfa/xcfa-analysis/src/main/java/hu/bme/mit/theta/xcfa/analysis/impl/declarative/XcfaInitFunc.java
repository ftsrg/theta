...
public class XcfaInitFunc<S extends ExprState, P extends Prec> implements InitFunc<XcfaState<S>, XcfaPrec<P>> {
	...
	public Collection<XcfaState<S>> getInitStates(final XcfaPrec<P> prec) {
		...
		for (S s : initFunc.getInitStates(prec.getGlobalPrec())) {
			final XcfaState<S> xcfaState = hu.bme.mit.theta.xcfa.analysis.impl.declarative.XcfaState.create(initLocs.stream().map(xcfaLocation -> Map.entry(xcfaLocation, true)).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue)), s);
			set.add(xcfaState);
		}
		return set;
	}
