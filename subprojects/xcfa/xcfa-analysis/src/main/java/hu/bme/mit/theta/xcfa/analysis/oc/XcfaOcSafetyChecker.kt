...
import hu.bme.mit.theta.analysis.algorithm.oc.SmtOcChecker
...
class XcfaOcSafetyChecker(
  ...
  private val ocChecker: XcfaOcChecker =
    decisionProcedure.checker(smtSolver, memoryModel).let { ocChecker ->
      if (conflictInput == null) ocChecker
        ...
        ocChecker as? SmtOcChecker<E>
          ?: error("OC correctness validator only supports SMT-based OC checkers.")
        XcfaOcCorrectnessValidator(ocChecker, conflictInput, !nonPermissiveValidation, logger)
      }
    }
  ...
  override fun check(prec: XcfaPrec<UnitPrec>?): SafetyResult<EmptyProof, Cex> =
    let {
        ...
        logger.info(
          "Auto conflict time (ms): " +
            measureTime {
                ...
                conflicts.forEach { ocChecker.addConflict(it) }
        ...
        ocChecker.printStatistics(logger)
