...
public final class CpuTimeKeeper {
    ...
    /**
     * Temporary solution: We cannot measure the time spent in child processes that are closed
     * already, so this function has to be called before closing the solvers (with
     * SolverManager.closeAll()) and it saves the cpu time of these processes, before they are
     * closed and adds it to the cpu time measured in getCurrentCpuTime()
     */
