...
class XcfaOcChecker(xcfa: XCFA, decisionProcedure: OcDecisionProcedureType, private val logger: Logger) :
    ...
    override fun check(
        prec: XcfaPrec<UnitPrec>?): SafetyResult<EmptyProof, Trace<XcfaState<out PtrState<*>>, XcfaAction>> = let {
        ...
        if (!addToSolver()) return@let SafetyResult.safe<EmptyProof, Trace<XcfaState<out PtrState<*>>, XcfaAction>>(
            EmptyProof.getInstance()) // no violations in the model
