...
class XcfaOcChecker(
  ...
  ): SafetyResult<EmptyWitness, Trace<XcfaState<out PtrState<*>>, XcfaAction>> =
    let {
        ...
        if (!addToSolver())
          return@let SafetyResult.safe<EmptyWitness, Trace<XcfaState<out PtrState<*>>, XcfaAction>>(
  ...
  private fun processThread(thread: Thread): List<Thread> {
    ...
    val newEvent: (VarDecl<*>, EventType) -> List<E> = { d, type ->
      ...
      events[decl] =
        (events[decl] ?: mutableMapOf()).apply {
          ...
          when (label) {
            is StmtLabel -> {
              when (val stmt = label.stmt) {
                ...
                is AssumeStmt -> {
                  ...
                  stmt.cond.vars.forEach { last = newEvent(it, EventType.READ) }
                  if (edge.source.outgoingEdges.size == 1 && asAssign) {
            ...
            is StartLabel -> {
              ...
              if (this.threads.any { it.pidVar == pidVar }) {
                error(
                  "Using a pthread_t variable in multiple threads is not supported by OC checker."
                )
              }
  ...
  private fun addCrossThreadRelations() {
    for ((_, map) in events) for ((pid1, list1) in map) for ((pid2, list2) in map) if (pid1 != pid2)
      for (e1 in list1.filter { it.type == EventType.WRITE }) for (e2 in
        list2.filter { it.type == EventType.READ }) rfs.add(RelationType.RF, e1, e2)
  }
