...
class XcfaOcChecker(
  ...
  override fun check(prec: XcfaPrec<UnitPrec>?): SafetyResult<EmptyWitness, Cex> =
    let {
            ...
            when (label) {
              ...
              is StartLabel -> {
                ...
                if (threads.any { it.pidVar == pidVar }) {
                  exit(
                    "Using a pthread_t variable in multiple threads: not supported by OC checker"
                  )
                }
                val newHistory = thread.startHistory + thread.procedure.name
                val newThread =
                  Thread(procedure, guard, pidVar, last.first(), newHistory, lastWrites)
