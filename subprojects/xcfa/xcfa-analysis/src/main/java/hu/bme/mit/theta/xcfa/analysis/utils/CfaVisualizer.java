...
//	}
//
//	public static void printTraceTable(final Trace<XcfaState<ExplState>, XcfaAction> trace, final TableWriter writer) {
//		final Set<Decl<?>> allVars = new LinkedHashSet<>();
//		for (final XcfaState<ExplState> state : trace.getStates()) {
//			allVars.addAll(state.getStates().getDecls());
//		}
...
//			final XcfaState<ExplState> state = trace.getState(i);
//			writer.cell(state.getLoc().getName());
//			for (final Decl<?> decl : allVars) {
//				final Optional<?> eval = state.getStates().eval(decl);
