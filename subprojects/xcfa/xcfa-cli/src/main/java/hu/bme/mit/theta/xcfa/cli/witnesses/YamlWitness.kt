...
val WitnessYamlConfig = Yaml(configuration = Yaml.default.configuration.copy(encodeDefaults = false))
 ...
 * For correctness:
 *  The `entry_type` defines what content is available in the content field. The only allowed entry type for a correctness witness is `invariant_set`.
 * @param entryType The `entry_type` defines what content is available in the content field. The only allowed entry type for a correctness witness is `violation_sequence`.
 * @param metadata  The meta data describe the provenance of the data.
 * @param content   For violations: The content represents the semantical content of the violation witness. It is a sequence of one or more segments. The content contains a sequence of zero or more normal segments and exactly one final segment at the end.
 *                  For correctness: The content represents the semantical content of the correctness witness. It is a sequence of one or more invariant elements.
 *
 */
@Serializable data class YamlWitness(
...
@Serializable enum class EntryType {
 ...
 * @param uuid  The `uuid` is a unique identifier of the entry; it uses the UUID format defined in RFC4122.
 * @param creationTime  The `creation_time` describes the date and time when the entry was created; it uses the format given by ISO 8601.
 ...
 * @param commandLine The command line with which the tool ran; it should be a bash-compliant command (optional)
 * @param description Any information not fitting in the previous items (optional)
 */
@Serializable data class Producer(
    val name: String,
    val version: String,
 ...
 * @param inputFiles The list of files given as input to the verifier, e.g. ["path/f1.c", "path/f2.c"].
 * @param inputFileHashes The SHA-256 hashes of all files in input_files, e.g. {"path/f1.c": 511..., "path/f2.c": f70...}
 * @param specification The specification considered by the verifier; it uses the SV-COMP format given at https://sv-comp.sosy-lab.org/2023/rules.php.
 * @param dataModel The data model assumed for the input program. (ILP32 / LP64)
 * @param language The programming language of the input files; the format currently supports only C.
 */
@Serializable data class Task(
    @SerialName("input_files")
    val inputFiles: List<String>,
    @SerialName("input_file_hashes")
    val inputFileHashes: List<String>,
    val specification: String,
    @SerialName("data_model")
    val dataModel: DataModel,
    val language: Language,
)

@Serializable enum class DataModel {
    ILP32,
    LP64,
}
 ...
 * @param location  The `location` is a mapping with mandatory keys file_name and line. All other keys are optional. If the location provides an inconsistent information (e.g., identifier specifies a function that is not called at the given column on the given line ), the witness is syntactically invalid.
 * @param action    Action `follow` means that the waypoint should be passed through. Action `avoid` means that the waypoint should be avoided.
 */
...
@Serializable enum class WaypointType {
 ...
 * @param format
 */
@Serializable data class Constraint(
    val value: String,
    val format: Format? = null,
)

@Serializable enum class Format {
    @SerialName("c_expression") C_EXPRESSION,
    @SerialName("acsl_expression") ACSL_EXPRESSION,
}

/**
 * The `location` is a mapping with mandatory keys file_name and line. All other keys are optional. If the location provides an inconsistent information (e.g., identifier specifies a function that is not called at the given column on the given line ), the witness is syntactically invalid.
 * @param fileName  The name of the file of the location
 * @param line The line number pf the program location in the file.
 * @param column The key `column` specifies the column number of the location, i.e., the position of the `column`-th character in the line that the location points to (value 1 is the position of the first character). If the column is not given, then it is interpreted as the leftmost suitable position on the line, where suitability is given by `type`.
 * @param function  The key `function` gives the name of the function containing the location. It is usually determined by the `file_name` and `line`, but the information can improve human readability of the witness.
 */
 ...
 */
@Serializable enum class Action {
    @SerialName("follow") FOLLOW,
    @SerialName("avoid") AVOID,
}

@Serializable data class Invariant(
...
@Serializable enum class InvariantType {
