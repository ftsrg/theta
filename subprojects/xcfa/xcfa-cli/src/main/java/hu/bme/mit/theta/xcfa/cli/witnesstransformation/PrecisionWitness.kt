      ...
      .flatMap {
        ...
        it.values.mapNotNull { value ->
          try {
            ...
            logger.writeln(INFO, "WARNING: Couldn't parse initial precision $value, skipping it (${e.message})")
            null
          }
        }
      }
...
private fun Collection<VarDecl<*>>.groupByScope(parseContext: ParseContext) = this
  .groupBy {
...
fun Iterable<VarDecl<*>>.filterInScope(scope: PrecisionScope, parseContext: ParseContext): List<VarDecl<*>> {
