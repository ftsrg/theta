...
public class ConversionInstructionHandler extends BaseInstructionHandler {
    ...
    private void trunc(
        ...
        foldExpression(
                ...
                blockState,
                op.getName(),
                functionState.getValues().get(op.getName()),
                0);
    }
    ...
    private void zext(
        ...
        Argument op = instruction.getArguments().get(0);
        checkState(
                op.getType() == IntType.getInstance() || op.getType() == BoolType.getInstance(),
                "Only integer/boolean values are allowed!");
        checkState(instruction.getRetVar().isPresent(), "Load must load into a variable");
        foldExpression(
                instruction,
                functionState,
                blockState,
                op.getName(),
                op.getType() == BoolType.getInstance()
                        ? Ite(
                                cast(
                                        functionState.getValues().get(op.getName()),
                                        BoolType.getInstance()),
                                IntLitExpr.of(BigInteger.ONE),
