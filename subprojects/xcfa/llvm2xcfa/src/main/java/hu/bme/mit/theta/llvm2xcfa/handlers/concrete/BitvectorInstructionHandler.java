...
public class BitvectorInstructionHandler extends BaseInstructionHandler {
    ...
    private void trunc(
        ...
        checkState(
                op1.getType() instanceof BvType,
                "Bitvector instructions only supports bitvector types!");
        checkState(instruction.getRetVar().isPresent(), "Instruction must have return variable");
        BigInteger newSize =
                BigInteger.valueOf(((BvType) instruction.getRetVar().get().getType()).getSize());
        ...
        foldExpression(
                ...
                null,
                BvExprs.Extract(
                        cast(op1.getExpr(functionState.getValues()), (BvType) op1.getType()),
                        IntLitExpr.of(oldSize.subtract(newSize)),
                        IntLitExpr.of(oldSize)),
                0);
    }
    ...
    private void zext(
        ...
        foldExpression(
                ...
                BvExprs.ZExt(
                        cast(op1.getExpr(functionState.getValues()), (BvType) op1.getType()),
                        (BvType) instruction.getRetVar().get().getType()),
                0);
    }
    ...
    private void bitcast(
        ...
        if (functionState.getLocalVars().containsKey(op1.getName())) {
            functionState
                    .getLocalVars()
                    .put(
                            instruction.getRetVar().get().getName(),
                            functionState.getLocalVars().get(op1.getName()));
        }
    }
    ...
    private void icmp(
        ...
        checkState(
                op2.getType() instanceof BvType, "Bitvector Icmp only supports bitvector types!");
        checkState(
                op3.getType() instanceof BvType, "Bitvector Icmp only supports bitvector types!");
        checkState(instruction.getRetVar().isPresent(), "Instruction must have return variable");
        switch (op1.getName()) {
            case "eq":
                foldExpression(
                        ...
                        BvExprs.Eq(
                                cast(
                                        op2.getExpr(functionState.getValues()),
                                        (BvType) op2.getType()),
                                cast(
    ...
    private void urem(
        ...
        foldExpression(
                ...
                null,
                BvExprs.URem(
                        cast(op1.getExpr(functionState.getValues()), (BvType) op2.getType()),
                        cast(op2.getExpr(functionState.getValues()), (BvType) op2.getType())),
                0);
    }
    ...
    private void mul(
        ...
        foldExpression(
                ...
                BvExprs.Mul(
                        List.of(
                                cast(
                                        op1.getExpr(functionState.getValues()),
                                        (BvType) op2.getType()),
                                cast(
                                        op2.getExpr(functionState.getValues()),
                                        (BvType) op2.getType()))),
    ...
    private void xor(
        ...
        if (op1.getType() instanceof BoolType && op2.getType() instanceof BoolType) {
            foldExpression(
                    ...
                    null,
                    BoolExprs.Xor(
                            cast(op1.getExpr(functionState.getValues()), (BoolType) op2.getType()),
                            cast(op2.getExpr(functionState.getValues()), (BoolType) op2.getType())),
                    0);
            return;
        }
        checkState(
