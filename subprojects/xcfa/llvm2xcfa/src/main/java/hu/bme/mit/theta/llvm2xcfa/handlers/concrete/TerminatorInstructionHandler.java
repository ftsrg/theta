    ...
    private void ret(
        ...
        switch (instruction.getArguments().size()) {
            ...
            case 1:
                ...
                Stmt assignStmt =
                        Assign(
                                cast(retVar, retVar.getType()),
                                cast(
                                        instruction
                                                .getArguments()
                                                .get(0)
                                                .getExpr(functionState.getValues()),
                                        retVar.getType()));
                stmts.add(assignStmt);
                break;
            default:
                throw new IllegalStateException(
                        "Unexpected value: " + instruction.getArguments().size());
        }
        XcfaEdge edge =
                new XcfaEdge(
                        blockState.getLastLocation(),
                        functionState.getProcedureBuilder().getFinalLoc().orElseThrow(),
                        new SequenceLabel(stmts.stream().map(stmt -> new StmtLabel(stmt)).toList()),
                        new LlvmMetadata(instruction.getLineNumber()));
        functionState.getProcedureBuilder().addEdge(edge);
        ...
        switch (instruction.getArguments().size()) {
            case 1:
                XcfaLocation loc =
                        functionState
                                .getLocations()
                                .get(instruction.getArguments().get(0).getName());
            ...
            case 3:
                XcfaLocation loc1 =
                        functionState
                                .getLocations()
                                .get(instruction.getArguments().get(1).getName());
                XcfaLocation loc2 =
                        functionState
                                .getLocations()
                                .get(instruction.getArguments().get(2).getName());
    ...
    private void sw(
        ...
        for (int i = 0; i < (instruction.getArguments().size() / 2) - 1; ++i) {
            XcfaLocation loc =
                    functionState
                            .getLocations()
                            .get(instruction.getArguments().get(2 + 2 * i + 1).getName());
            Expr<?> constExpr =
                    instruction.getArguments().get(2 + 2 * i).getExpr(functionState.getValues());
            checkState(
                    varExpr.getType().equals(constExpr.getType()),
                    "variable and constant should be of the same type!");
            EqExpr<?> eq =
                    AbstractExprs.Eq(
                            cast(varExpr, constExpr.getType()),
                            cast(constExpr, constExpr.getType()));
        ...
        XcfaLocation loc =
                functionState.getLocations().get(instruction.getArguments().get(1).getName());
        XcfaEdge edge =
                new XcfaEdge(
                        blockState.getLastLocation(),
                        loc,
                        new StmtLabel(Assume(BoolExprs.Not(defaultBranch))),
                        new LlvmMetadata(instruction.getLineNumber()));
        functionState.getProcedureBuilder().addEdge(edge);
        blockState.setLastLocation(functionState.getProcedureBuilder().getFinalLoc().get());
    }
