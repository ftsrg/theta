...
public class MemoryInstructionHandler extends BaseInstructionHandler {
    ...
    private void load(
        ...
        if (isatomic.getName().equals("atomic")) op = instruction.getArguments().get(2);
        else op = instruction.getArguments().get(1);
        checkState(
                functionState.getLocalVars().containsKey(op.getName()),
                "Load must load a variable!");
        ...
        foldExpression(
                ...
                blockState,
                op.getName(),
                Utils.getOrCreateVar(functionState, op).getRef(),
                1);
    }
    ...
    private void store(
        ...
        Tuple2<VarDecl<?>, Integer> potentialParam =
                functionState.getLocalVars().get(op1.getName());
        checkState(
                functionState.getLocalVars().containsKey(op2.getName())
                        || functionState.getParams().contains(oldVar.get1()),
                "Store must store into a variable!");

        if (oldVar.get2() > 1) {
            functionState
                    .getLocalVars()
                    .put(op2.getName(), Tuple2.of(oldVar.get1(), oldVar.get2() - 1));
        } else if (oldVar.get2() == 1) {
            if (potentialParam != null
                    && functionState.getParams().contains(potentialParam.get1())) {
                ...
                XcfaLocation loc =
                        new XcfaLocation(blockState.getName() + "_" + blockState.getBlockCnt());
                VarDecl<?> var = functionState.getLocalVars().get(op2.getName()).get1();
                Stmt stmt =
                        Assign(
                                cast(var, var.getType()),
                                cast(op1.getExpr(functionState.getValues()), var.getType()));
                XcfaEdge edge =
                        new XcfaEdge(
