...
public class XcfaUtils {
    ...
    public static XCFA fromFile(File model, ArithmeticType arithmeticType) throws IOException {
        ...
        if (model.getName().endsWith("ll") || model.getName().endsWith("bc")) {
            ...
            return createXCFA(
                    new LlvmIrProvider(model.getAbsolutePath(), true, true, true, true),
                    arithmeticType);
            ...
            if (split.length > 0)
                throw new RuntimeException(
                        "File type " + split[split.length - 1] + " not supported.");
     ...
     */
    public static XCFA createXCFA(
            SSAProvider ssa, List<ProcedurePass> procedurePasses, ArithmeticType arithmeticType) {
        if (ssa.hasStructs()) {
        ...
        checkState(
                !ssa.shouldUseBitwiseArithmetics() || arithmeticType == ArithmeticType.bitvector,
                "There are statements in the source not mappable to integer arithmetic");
        ...
        for (Tuple3<String, Optional<String>, List<Tuple2<String, String>>> function :
                ssa.getFunctions()) {
            ...
            for (String block : ssa.getBlocks(function.get1())) {
                ...
                InstructionHandlerManager instructionHandlerManager =
                        new InstructionHandlerManager(arithmeticType);
                for (Tuple4<
                                String,
                                Optional<Tuple2<String, String>>,
                                List<Tuple2<Optional<String>, String>>,
                                Integer>
                        instruction : ssa.getInstructions(function.get1(), block)) {
                    try {
                        InstructionHandler instructionHandler =
                                instructionHandlerManager.createInstructionHandlers();
    ...
    //                                                                (Function<XcfaProcess,
    // XcfaProcedure>)
    //                                                                (XcfaProcess xcfaProc) -> new
    // XcfaProcedure(
    //                                                                        e.getKey(),
    //                                                                        Map.of(),
    //
