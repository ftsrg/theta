...
class CatVisitor(file: File) : CatBaseVisitor<GraphPattern>() {
  ...
  override fun visitProcCall(ctx: ProcCallContext): GraphPattern {
    ...
    Preconditions.checkNotNull(
      procDefContext,
      "Procedure with name " + ctx.NAME().text + " does not exist.",
    )
    val e = ctx.params
  ...
  override fun visitExprFunctionCall(ctx: ExprFunctionCallContext): GraphPattern {
    ...
    Preconditions.checkNotNull(
      functionDefContext,
      "Function with name " + ctx.NAME().text + " does not exist.",
    )
    val e = ctx.e
    val toReset: MutableMap<String, GraphPattern?> = LinkedHashMap()
    for (i in e.indices) {
  ...
  override fun visitExprBasic(ctx: ExprBasicContext): GraphPattern {
    return getOrCreateRelation(
      ctx.NAME().text,
      if (Character.isLowerCase(ctx.NAME().text[0])) 2 else 1,
    )!!
  }
  ...
  override fun visitExprMinus(ctx: ExprMinusContext): GraphPattern {
    ...
    return if (rel1 is EdgePattern && rel2 is EdgePattern) Difference(rel1, rel2)
    else if (rel1 is NodePattern && rel2 is NodePattern) DifferenceNode(rel1, rel2)
  ...
  override fun visitExprUnion(ctx: ExprUnionContext): GraphPattern {
    val rel1 = ctx.e1.accept(this)
    val rel2 = ctx.e2.accept(this)
    return if (rel1 is EdgePattern && rel2 is EdgePattern) Union(rel1, rel2)
    else if (rel1 is NodePattern && rel2 is NodePattern) UnionNode(rel1, rel2)
    else error("Mismatched types")
  }
  ...
  override fun visitExprIntersection(ctx: ExprIntersectionContext): GraphPattern {
    val rel1 = ctx.e1.accept(this)
    val rel2 = ctx.e2.accept(this)
    return if (rel1 is EdgePattern && rel2 is EdgePattern) Intersection(rel1, rel2)
    else if (rel1 is NodePattern && rel2 is NodePattern) IntersectionNode(rel1, rel2)
    else error("Mismatched types")
  }
  ...
  override fun visitExprComplement(ctx: ExprComplementContext): GraphPattern {
    ...
    return if (rel is EdgePattern) Complement(rel)
    else if (rel is NodePattern) ComplementNode(rel) else error("Mismatched types")
  }
