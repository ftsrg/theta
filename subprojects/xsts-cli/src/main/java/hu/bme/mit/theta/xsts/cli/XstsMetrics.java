...
public final class XstsMetrics {
	...
	public static void printMetrics(Logger logger, XSTS xsts){
		...
		logger.write(Logger.Level.RESULT, "  Bool vars: %s%n" , xsts.getVars().stream().filter(v -> v.getType() instanceof BoolType).count());
		logger.write(Logger.Level.RESULT, "  Int vars: %s%n" , xsts.getVars().stream().filter(v -> v.getType() instanceof IntType).count());
		...
		logger.write(Logger.Level.RESULT, "  Array vars: %s%n" , xsts.getCtrlVars().size());
