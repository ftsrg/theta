...
public final class XtaCli {
    ...
    @Parameter(
            names = {"--discreteconcr", "-dc"},
            description = "Concrete domain for discrete variables",
            required = false)
    DataStrategy.ConcrDom concrDataDom = DataStrategy.ConcrDom.EXPL;

    @Parameter(
            names = {"--discreteabstr", "-da"},
            description = "Abstract domain for discrete variables",
            required = false)
    DataStrategy.AbstrDom abstrDataDom = DataStrategy.AbstrDom.EXPL;

    @Parameter(
            names = {"--discreteitp", "-di"},
            description = "Interpolation strategy for discrete variables",
            required = false)
    DataStrategy.ItpStrategy dataItpStrategy = DataStrategy.ItpStrategy.BIN_BW;

    @Parameter(
            names = {"--meet", "-me"},
            description = "Meet strategy for expressions",
            required = false)
    ...
    @Parameter(
            names = {"--combined-dataDomain"},
            description = "Abstract domain for data")
    CombinedLazyCegarXtaCheckerConfigFactory.DataDomain dataDomain =
            CombinedLazyCegarXtaCheckerConfigFactory.DataDomain.EXPL;

    @Parameter(
            names = "--combined-predsplit",
            description = "Predicate splitting (for predicate abstraction)")
    CombinedLazyCegarXtaCheckerConfigFactory.PredSplit predSplit =
            CombinedLazyCegarXtaCheckerConfigFactory.PredSplit.WHOLE;

    @Parameter(
            names = "--combined-maxenum",
            description = "Maximal number of explicitly enumerated successors (0: unlimited)")
    Integer maxEnum = 10;

    @Parameter(
            names = {"--combined-dataRefinement"},
            description = "Data refinement strategy")
    CombinedLazyCegarXtaCheckerConfigFactory.DataRefinement dataRefinement =
            CombinedLazyCegarXtaCheckerConfigFactory.DataRefinement.SEQ_ITP;

    @Parameter(
            names = {"--combined-pruneStrategy"},
    ...
    private SafetyResult<? extends ARG<?, ?>, ? extends Trace<? extends State, ? extends Action>>
        ...
        final CombinedLazyCegarXtaCheckerConfig<
                        ?,
                        ?,
                        ? extends ARG<?, ?>,
    ...
    private void printResult(
            final XtaSystem.PropertyKind propertyKind,
