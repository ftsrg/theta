...
public final class XtaDataAction extends StmtAction {
                ...
                .forEachOrdered(
                        c ->
                                c.stream()
                                        .forEachOrdered(
                                                nonRecvEdge ->
                                                        addDisabledNonRecvEdges(
                                                                builder, emitEdge, nonRecvEdge)));
        addInvariants(builder, xtaAction.getTargetLocs());
        return builder.build();
    }

    private void addGuards(final ImmutableList.Builder<Stmt> builder, final Edge edge) {
        edge.getGuards().stream()
                .filter(Guard::isDataGuard)
                .forEach(guard -> builder.add(Assume(guard.toExpr())));
    }

    private void addUpdates(final ImmutableList.Builder<Stmt> builder, final Edge edge) {
        edge.getUpdates().stream()
                .filter(Update::isDataUpdate)
                .forEach(update -> builder.add(update.toStmt()));
    }

    private void addInvariants(
            final ImmutableList.Builder<Stmt> builder, final Collection<Loc> locs) {
        locs.forEach(
                loc ->
                        loc.getInvars().stream()
                                .filter(Guard::isDataGuard)
                                .forEach(invar -> builder.add(Assume(invar.toExpr()))));
    }

    private void addSync(
            final ImmutableList.Builder<Stmt> builder, final Edge emitEdge, final Edge recvEdge) {
    ...
    private void addDisabledNonRecvEdges(
