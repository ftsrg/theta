...
public final class XtaExplUtils {
    ...
    private static ExplState postForBroadcastAction(
        ...
        if (recvEdges.stream().anyMatch(recvEdge -> !checkSync(emitEdge, recvEdge, succVal))) {
            return ExplState.bottom();
        }
        ...
        if (recvEdges.stream().anyMatch(recvEdge -> !checkGuards(recvEdge, succVal))) {
            return ExplState.bottom();
        }
