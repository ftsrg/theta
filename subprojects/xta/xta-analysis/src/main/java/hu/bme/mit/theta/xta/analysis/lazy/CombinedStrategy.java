...
final class CombinedStrategy<S1 extends State, S2 extends State>
    ...
    public CombinedStrategy(
        ...
        projection =
                s ->
                        Tuple3.of(
                                strategy1.getProjection().apply(s.getState().getState1()),
                                strategy2.getProjection().apply(s.getState().getState2()),
                                s.getLocs());
    }
    ...
    public boolean mightCover(
            final ArgNode<XtaState<Prod2State<S1, S2>>, XtaAction> coveree,
    ...
    public void cover(
            ...
            final Builder stats) {
        assert coveree.getCoveringNode().isPresent()
                && coveree.getCoveringNode().get().equals(coverer);
    ...
    @Override
    public void block(
            final ArgNode<XtaState<Prod2State<S1, S2>>, XtaAction> node,
    ...
    private Analysis<XtaState<Prod2State<S1, S2>>, XtaAction, UnitPrec> createAnalysis(
        ...
        final Analysis<Prod2State<S1, S2>, XtaAction, Prod2Prec<UnitPrec, UnitPrec>> prodAnalysis =
                Prod2Analysis.create(analysis1, analysis2);
        final Analysis<XtaState<Prod2State<S1, S2>>, XtaAction, Prod2Prec<UnitPrec, UnitPrec>>
                xtaAnalysis = XtaAnalysis.create(system, prodAnalysis);
        final Prod2Prec<UnitPrec, UnitPrec> prec =
                Prod2Prec.of(UnitPrec.getInstance(), UnitPrec.getInstance());
        final Analysis<XtaState<Prod2State<S1, S2>>, XtaAction, UnitPrec> analysis =
                PrecMappingAnalysis.create(xtaAnalysis, p -> prec);
        return analysis;
    }
}
