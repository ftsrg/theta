...
public final class XtaClockPredUtils {

    private XtaClockPredUtils() {
    }

    public static ZoneState post(final ZoneState state, final XtaAction action, final ClockPredPrec prec) {
        checkNotNull(state);
        checkNotNull(action);
        checkNotNull(prec);
    ...
    private static ZoneState postForBroadcastAction(final ZoneState state, final BroadcastXtaAction action,
        ...
        if (recvEdges.stream().anyMatch(XtaClockPredUtils::hasClockGuards)) {
            throw new UnsupportedOperationException(
                    "Clock guards on edges with broadcast synchronization labels are not supported.");
        }

        if (nonRecvEdgeCols.stream().anyMatch(c -> c.stream().anyMatch(XtaClockPredUtils::hasClockGuards))) {
            throw new UnsupportedOperationException(
                    "Clock guards on edges with broadcast synchronization labels are not supported.");
        }
    ...
    public static ZoneState pre(final ZoneState state, final XtaAction action, final ClockPredPrec prec) {
        checkNotNull(state);
        checkNotNull(action);
        checkNotNull(prec);
    ...
    private static ZoneState preForBroadcastAction(final ZoneState state, final BroadcastXtaAction action,
        ...
        if (reverseRecvEdges.stream().anyMatch(XtaClockPredUtils::hasClockGuards)) {
            throw new UnsupportedOperationException(
                    "Clock guards on edges with broadcast synchronization labels are not supported.");
        }
