...
/**
 * The XCFA memory model is a two-dimensional array -- {@code arrays[base][offset]} -- so a model
 * for it has array-valued entries, and a byte-addressed union is the thing that fills such an array
 * cell-by-cell. Extracting one used to throw a bare {@code NullPointerException}: {@code
 * createEntryExprs} / {@code toArrayLitExpr} put a null into {@code Tuple2.of(args, value)}
 * whenever a nested array term Z3 left uninterpreted transformed to null. These are smoke tests
 * over that extraction path; the exact production crash is reproduced end to end (a counterexample
 * over a plain {@code unsigned char[16]}, which turned into a NullPointerException before the fix
 * and into a clean Unsafe verdict after), since the precise Z3 model shape that trips it is hard to
 * force through the bare solver API.
 */
public final class NestedArrayModelTest {
    ...
    public void distinctRowsWithUnconstrainedInnerArraysExtract() {
        ...
        final Solver solver = Z3LegacySolverFactory.getInstance().createSolver();
        // Two outer rows must differ while no inner cell is pinned -- the shape most likely to
        // leave
