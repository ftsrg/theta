...
public final class SMTInterpolSmtLibItpSolver extends SmtLibItpSolver<SMTInterpolSmtLibItpMarker> {
    ...
    @Override
    public SMTInterpolSmtLibItpMarker createMarker() {
        final var marker = new SMTInterpolSmtLibItpMarker();
        markers.add(marker);
        return marker;
    }
    ...
    public Interpolant getInterpolant(final ItpPattern pattern) {
        ...
        buildItpMapFormList(smtInterpolItpPattern.getRoot(), itpList, itpMap);

        return new SmtLibInterpolant(itpMap);
    }

    private String patternToTerm(final ItpMarkerTree<SMTInterpolSmtLibItpMarker> markerTree) {
        String term;
