...
public enum EnumStrategy {
    ...
    DATATYPES {
        @Override
        public void declareDatatypes(final Collection<Type> allTypes, final Stack<EnumType> typeStack, final Consumer<String> issueGeneralCommand) {
            ...
            for (Type t :
                ...
                if (t instanceof EnumType enumType) {
                    ...
                    issueGeneralCommand.accept(String.format("(declare-datatypes ((%s 0)) ((%s)))", enumType.getName(), String.join(" ", literals)));
                }
            }
        }
    ...
    SORTS {
        @Override
        public void declareDatatypes(Collection<Type> allTypes, Stack<EnumType> typeStack, Consumer<String> issueGeneralCommand) {
            List<Type> types = new ArrayList<>(allTypes);
            types.removeAll(typeStack.toCollection());
            Set<Type> typeSet = new HashSet<>(types);
            for (Type t :
                ...
                if (t instanceof EnumType enumType) {
                    typeStack.add(enumType);
                    issueGeneralCommand.accept(String.format("(declare-sort %s 0)", enumType.getName()));
                    enumType.getLongValues().forEach(literal -> issueGeneralCommand.accept(String.format("(declare-const %s %s)", literal, enumType.getName())));
                    issueGeneralCommand.accept(String.format("(assert (distinct %s))", String.join(" ", enumType.getLongValues())));
                }
            }
        }
        ...
        public String wrapAssertionExpression(String assertion, Map<ConstDecl<?>, String> consts) {
            ...
            for (var constDeclEntry : consts.entrySet()) {
                ...
                if (constDecl.getType() instanceof EnumType enumType) {
                    sb.append(" (or");
                    enumType.getLongValues().forEach(val -> sb.append(String.format(" (= %s %s)", nameOnSolver, val)));
                    sb.append(")");
                    needsWrap = true;
                }
            }
        ...
        public EnumLitExpr transformEnumTerm(Function_defContext funcDef, EnumType type, SmtLibModel model) {
            ...
            for (var lit : type.getLongValues()) {
                if (model.getTerm(lit).contains(id))
                    return type.litFromLongName(lit);
            }
    ...
    public abstract void declareDatatypes(final Collection<Type> allTypes, final Stack<EnumType> typeStack, final Consumer<String> issueGeneralCommand);
     ...
     */
    public String wrapAssertionExpression(final String assertion, final Map<ConstDecl<?>, String> consts) {
        return assertion;
    }
    ...
    public static EnumStrategy getDefaultStrategy() {
