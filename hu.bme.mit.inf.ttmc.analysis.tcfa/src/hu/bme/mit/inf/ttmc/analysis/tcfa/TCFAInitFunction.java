...
import static hu.bme.mit.inf.ttmc.common.Unit.unit;
...
public class TCFAInitFunction<S extends State, P extends Precision> implements InitFunction<TCFAState<S>, P, TCFALoc> {
	...
	private final InitFunction<S, P, Unit> initFunction;

	public TCFAInitFunction(final InitFunction<S, P, Unit> initFunction) {
	...
	public Collection<TCFAState<S>> getInitStates(final P precision, final TCFALoc initLoc) {
		...
		final Collection<? extends S> subInitStates = initFunction.getInitStates(precision, unit());
