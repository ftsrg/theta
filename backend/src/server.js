            ...
            if (ent.name.endsWith('.dot') && fs.existsSync(runExec)) {
              ...
              try {
                ...
                if (dotResult.code === 0 && fs.existsSync(svgPath)) {
                  ...
                  throw new Error('dot output not found, exit code ' + dotResult.code + ', stdout: ' + dotResult.stdout + ', stderr: ' + dotResult.stderr + ', output: ' + fsp.readFile(path.join(runDir, 'output.log'), 'utf8').catch(()=>'<unreadable>'));
                }
              } catch (dotErr) {
              ...
              if (fs.existsSync(linterScript) && fs.existsSync(srcFile)) {
                ...
                try {
                  ...
                  if (fs.existsSync(lintOutputPath)) {
                    ...
                    throw new Error('linter output not found, exit code ' + lintResult.code + ', stdout: ' + lintResult.stdout + ', stderr: ' + lintResult.stderr + ', output: ' + fsp.readFile(path.join(runDir, 'output.log'), 'utf8').catch(()=>'<unreadable>'));
                  }
                } catch (lintErr) {
