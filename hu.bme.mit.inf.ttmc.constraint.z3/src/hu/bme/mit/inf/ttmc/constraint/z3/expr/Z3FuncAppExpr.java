...
public final class Z3FuncAppExpr<ParamType extends Type, ResultType extends Type>
	...
	public Z3FuncAppExpr(final ConstraintManager manager,
	...
	//
	// if (term == null) {
	// final Tuple2<ConstRefExpr<?>, List<Expr<?>>> extractedExprs =
	// extractExprs(this);
	...
	// final com.microsoft.z3.Expr[] paramTerms = new
	// com.microsoft.z3.Expr[params.size()];
	//
	// int i = 0;
	...
	//
	// final Tuple2<ConstRefExpr<?>, List<Expr<?>>> subResult =
	// extractExprs(func);
	// final List<Expr<?>> params = subResult._2();
	//
	// final ConstRefExpr<?> constRef = subResult._1();
	// final List<Expr<?>> newParams = ImmutableList.<Expr<?>>
	// builder().addAll(params).add(param).build();
	//
