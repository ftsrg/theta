...
import hu.bme.mit.theta.formalism.sts.analysis.initprec.StsEmptyInitPrec;
import hu.bme.mit.theta.formalism.sts.analysis.initprec.StsInitPrec;
import hu.bme.mit.theta.formalism.sts.analysis.initprec.StsPropInitPrec;
...
public final class StsConfigBuilder {
	...
	public StsConfigBuilder solverFactory(final SolverFactory solverFactory) {
	...
	public Config<? extends State, ? extends Action, ? extends Prec> build(final STS sts) {
		...
		if (domain == Domain.EXPL) {
			...
			final ArgBuilder<PredState, StsAction, PredPrec> argBuilder = ArgBuilder.create(lts, analysis,
					target);
			...
			final Refiner<PredState, StsAction, PredPrec> refiner = SingleExprTraceRefiner.create(
					exprTraceChecker, JoiningPrecRefiner.create(new ItpRefToPredPrec(solver, predSplit.splitter)),
					logger);
