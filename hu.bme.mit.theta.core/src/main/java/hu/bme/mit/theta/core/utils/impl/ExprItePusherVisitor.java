...
public class ExprItePusherVisitor extends ArityBasedExprVisitor<Void, Expr<? extends Type>> {
	...
	protected <OpType extends Type, ExprType extends Type> Expr<? extends Type> visitUnary(final UnaryExpr<OpType, ExprType> expr, final Void param) {
		...
		if (!(expr.getOp() instanceof IteExpr) || expr.getOp().getType() instanceof BoolType) {
			...
			return Exprs.Ite(op.getCond(), expr.withOp(op.getThen()).accept(this, param), expr.withOp(op.getElse()).accept(this, param));
		}
	...
	protected <LeftOpType extends Type, RightOpType extends Type, ExprType extends Type> Expr<? extends Type> visitBinary(
		...
		if (!(expr.getLeftOp() instanceof IteExpr || expr.getRightOp() instanceof IteExpr)) {
			...
			return Exprs.Ite(op.getCond(), expr.withLeftOp(op.getThen()).accept(this, param), expr.withLeftOp(op.getElse()).accept(this, param));
			...
			return Exprs.Ite(op.getCond(), expr.withRightOp(op.getThen()).accept(this, param), expr.withRightOp(op.getElse()).accept(this, param));
		} else {
			return expr;
		}
	}
	...
	protected <OpsType extends Type, ExprType extends Type> Expr<? extends Type> visitMultiary(final MultiaryExpr<OpsType, ExprType> expr, final Void param) {
		...
		final Optional<? extends Expr<? extends OpsType>> firstIte = expr.getOps().stream().filter(op -> op instanceof IteExpr).findFirst();
		// Nothing to do if the none of the operands are ITE or it is of bool
		// type
		if (!firstIte.isPresent() || expr.getOps().size() == 0 || expr.getOps().iterator().next().getType() instanceof BoolType) {
			return expr;
		} else {
			...
			return Exprs.Ite(ite.getCond(), expr.withOps(thenOps).accept(this, param), expr.withOps(elseOps).accept(this, param));
		}
	}
