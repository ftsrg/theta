...
public final class Env {
	...
	public Object compute(final String symbol, final Function<? super String, ? extends Object> mapping) {
		checkNotNull(symbol);
	...
	private static final class Frame {
		...
		public void define(final String symbol, final Object value) {
			checkArgument(!symbolToValue.containsKey(symbol), "Symbol   \"" + symbol + "\" is already defined");
		...
		public String toString() {
			return Utils.lispStringBuilder(getClass().getSimpleName()).aligned().addAll(
					symbolToValue.entrySet().stream().map(e -> String.format("(%s %s)", e.getKey(), e.getValue())))
					.toString();
		}
	}
