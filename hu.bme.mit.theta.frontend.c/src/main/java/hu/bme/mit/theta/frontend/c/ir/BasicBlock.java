...
public class BasicBlock {
	 ...
	 *
	 * @param name
	 *            A label referring to this block
	 * @param nodes
	 *            A list of non-terminating instruction nodes
	 */
	 ...
	 *
	 * @param function
	 *            A function object which should contain this block
	 */
		...
		if (oldNode == this.terminator) {
			...
			if (!term.getTargets().equals(this.terminator.getTargets()))
				throw new IllegalArgumentException(
						"Use clearTerminator and terminate to replace a terminator node with a terminator of different targets");
	 ...
	 *
	 * This operation throws an exception if the node is not contained in the
	 * block
	 *
	 * @param node
	...
	public void removeNode(int idx) {
		if (idx < 0 || idx >= this.nodes.size())
			throw new IllegalArgumentException(
					"The block '" + this.name + "' does not contain the given node index: " + idx);
	 ...
	 *
	 * If the block is already terminated, this operation fails with an
	 * exception. Use clearTerminator to reset the block terminator.
	 */
	 ...
	 *
	 * @param block
	 *            A block which has an edge into this block
	 */
